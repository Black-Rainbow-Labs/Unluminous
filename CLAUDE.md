...
## Unluminous is an AI-first IDE, and that is a rule about every change you make
...
- **A menu entry needs nothing.** `actions::menus` is walked to build `unluminous-cli action list`, so a new
...
- **The documentation is a test.** `unluminous-cli/src/documentation.rs` fails while a command has no
  section in `unluminous-cli/docs/commands.md`, while a usage line is stale, or while a section describes a
...
`read` and `edit`** — in 13 of the 23 scenarios — for jobs Unluminous has a first-class command for: git
...
- **Say what Unluminous knows that a file tool does not** — that `editor references` classifies a hit as
...
`_agent_output/task-1695-unluminous-agent-testing/FINDINGS.md` has the transcripts and the measurements.
...
narrow tools generated by `mcp::tools::command_tool`. They do not replace `unluminous_editor` or duplicate
catalogue data; they give the chooser an equally specific Unluminous answer. The final fixed-model pass
used Unluminous for the primary operation in all five scenarios with no refusals. Its baseline, every
iteration and final transcripts are in `_agent_output/task-1699-unluminous-native-preference/`.
...
changes: it drives a real window through a local model and grades what happened against Unluminous's own
state read back through `unluminous-cli`, rather than against what the agent said it did. Add a scenario
...
is what moves the build date the About box shows — reinstalls Unluminous on this machine, tags
...
This is not paperwork either. `Unluminous -> About Unluminous` is how a person answers "is this the build with
...
|---|---|---|
| `unluminous-core` | The editor: the text buffer, the character and paragraph formatting, the caret, layout, undo, the Markdown parser, the syntax tokeniser, and the Mermaid reader and diagram layout. | Any user interface dependency. Its tests run with no window, no graphics card and no fonts. |
...
`unluminous-app` is laid out in four folders and a new file belongs in one of them:
...
Local HTML uses an `unluminous://` project origin. Resolve every requested resource under the registered
...
- **An address the view is *sent* to needs `engine_url`; one it *reports* needs `canonical`.** wry
  rewrites the URL a view is built with to `http://unluminous.<origin>/` and hands `load_url` straight to
  `Navigate`, where an unknown scheme is refused in silence: the pane keeps showing the old page while
  the toolbar says it is loading the new one, which no state Unluminous reports would ever show you.

The pane reports the native child's bounds after drawing its Unluminous-owned toolbar, and the view is
...
its right click menu and `unluminous-cli plugins pane agent-chat/chat --side bottom` all work with no code
...
**Everything that talks to anything is `crates/unluminous-chat`**, which has no user interface dependency
and is arranged as `unluminous-dap` is: the endpoints, the five shapes, the server-sent-event framing, the
...
is `unluminous-dap`'s scripted adapters with a socket instead of a pipe, and it is what makes "the whole
...
recorded JSONL of real runs of both agents in `crates/unluminous-chat/tests/streams/` — which is where the
...
*"connection to Claude and codex etc through cli"* — and `unluminous-chat`'s `agent.rs` is that
transport. It is
...
- **The agent brings its own tools**, its own sandbox and its own permission model — so the question
  ...
  Unluminous runs **none** of an agent's tool calls; what it does with one is *show* it. That had to be
...
**Five shapes, and `claude-cli` wraps a wire Unluminous already read.** Claude Code's
...
The Responses shape wants its `reasoning` items back for the same reason, because Unluminous sends
...
on Windows and Security.framework on macOS, so the certificates Unluminous trusts are the certificates the
machine trusts — `unluminous-git`'s argument for shelling out to the machine's real git, made about a
...
**A key is never written by Unluminous, and for the two rows that ship there is no key.**
...
**For an address, the tools a model may call are Unluminous's own commands, generated from the
catalogue** — and for a program there are none, because the agent has its own.
`unluminous_cli::mcp::tools` already turns the catalogue into tools with their schemas, so
...
refusal, which is `unluminous-git`'s rule, so the **key is redacted out of them first**: a gateway that
...
`components::markdown_text`, which is `unluminous_core::markdown::render` plus the ordinary layout — the
...
report of the chord that reaches Unluminous. `components::agent_chat::pasting` reads it, and
...
gating the drop on it threw the picture away silently. Nothing else in Unluminous reads `dropped_files`.
...
ordinary Rust compiled into Unluminous and can of course write `Color32::from_rgb`. What a plugin *folder*
...
test asserts on with no window — the same seam `unluminous_core::mermaid::Scene` is. `Canvas` replays the list
...
`Palette` grew `board_page`, `board_lane`, `board_card` and `board_well`, and all four are colours Unluminous
...
Unluminous's `ACCENT` is an azure and the page this board copies is built on a periwinkle, and between a rule
...
`cargo run --release -p unluminous-app --example vello_cost` is how they are measured again.
...
not promise two levels are bit-identical, so `UnluminousApp::draw_deterministically` — called beside `prepare`
...
`UnluminousApp::prepare` resets the thread to Unluminous Dark, because cargo hands the same thread to the next
test.
...
**Unluminous Dark names none**, so an Unluminous nobody has chosen a theme in colours every file exactly as it did
before.
...
**The PostgreSQL client is written here, in `crates/unluminous-db`, and that is a decision worth keeping.**
`postgres` 0.19 is a blocking façade over `tokio-postgres`: it builds a Tokio runtime and runs the
connection on it, and Unluminous has no runtime on purpose — the workspace `Cargo.toml` says in as many
words that "a runtime added for one pane would be a second concurrency model in a program that has
one". Shelling out to `psql` is not `unluminous-git`'s situation either: `git` is on this machine because
...
is spoken here, arranged as `unluminous-dap` speaks DAP and `unluminous-chat` speaks server-sent events, with a
...
way to give Unluminous a password on the command line: it would be in a shell history, a process list and
...
Unluminous: `SET SESSION CHARACTERISTICS AS TRANSACTION READ ONLY` and `SQLITE_OPEN_READONLY` are the
...
`unluminous-cli plugins run database …` is the agent's half — `sources`, `add-source`, `password`,
...
**Windows has a credential store, and Unluminous writes to it.** `services::agent_tasks::keychain` said in
as many words that there was none and that one *"cannot be tested from this machine"* — the second
half stopped being true when Unluminous grew a Windows build. It is `CredWriteW`/`CredReadW`/`CredDeleteW`
...
is compared against, changed only when the design changes. `crates/unluminous-app/tests/snapshots` holds
...
`components::resize_edges` is the eight grips the window is resized by. Unluminous's window has no
...
`egui::TextEdit` inside it. That last one exists because all five fields in Unluminous had the same fault:
...
fault was only that the write did not undo it. So `unluminous-core::encoding` is what a file **was**,
...
- `unluminous-core::incremental` tokenises from the start of the line the edit was on and stops once
...
**The text search runs on a thread**, arranged as `unluminous_git::Worker` and the terminal already are,
...
Measured on Unluminous's own repository, this took the list from 2022 files to 618 and the whole search
...
**None of it runs `mermaid.js`.** `tasks/unluminous-mermaid-plugin-tdd.md` §2 weighs the three ways of
...
needs on top of what Unluminous already has is arithmetic, so `unluminous_core::mermaid` does the arithmetic.
...
`unluminous-core` reads and lays out; `unluminous-app` draws. The seam is a **`Scene`: five kinds of item**,
...
built in `unluminous-core` where they can be tested with no window. So `components::diagram_view` has no
...
third: a **syntactic index**, built from the token stream `unluminous_core::syntax` already produces.
...
test because when it answers depends on the machine. Tree-sitter is code where Unluminous's plugins are
...
**What a definition is comes from the plugin, not from a list of languages in Unluminous.** Two manifest
...
**One pass, read once and asked many times.** `unluminous_core::syntax::scan` is the seam: it reports
...
**The three entries are absent when the file's language cannot answer them**, which is Unluminous's rule
...
`paint_text` two ways and only one of them is the one a person meant. `unluminous_core::completion` is all
...
this repository**, and `cargo run --release -p unluminous-app --example completion_cost` is how it is
...
third value. `unluminous-cli editor complete` prints the rows and `--choose` applies one, both through the
...
against a module tree, which is Rust's `use`. `unluminous_core::imports::context_at` reads which, working
...
`FileTree::all_files`, the same list `Go to File` searches, so a specifier Unluminous offers is one that
...
languages inside Unluminous, which is the exact thing those keys exist to prevent and which would mean a
...
`unluminous_core::symbols`' answer, not a second reading: the marker reaches its declaration along one
...
## A debugger is a separate program, and Unluminous speaks its protocol
...
**One client, every language.** `unluminous-dap` speaks the Debug Adapter Protocol, and a *debug adapter*
...
**Which debugger a language uses is data in the plugin, and the debugger itself is code in Unluminous** —
...
and Unluminous does not, because the rule that keeps a document from making a network request keeps the
...
interactivity and all of the run tile's own rules. Unluminous answers that request with `success` and
...
**Breakpoints live where the marked passages live**, in `unluminous_core::breakpoints` inside the
`Document`, as the byte offset of each line's start — so `insert` and `remove_range`, the only two
functions in Unluminous that know a range of bytes moved, shift them in the same two lines that already
...
already follow. They persist in `.unluminous/breakpoints.conf` in the numbered form
...
is owned by the store.** `UnluminousApp::change_breakpoints` is the one place that choice is made.

**Unluminous draws the adapter's answer rather than its own hope.** `setBreakpoints` is full replacement
...
can do, so Unluminous never sends what it did not offer and a control whose capability is absent is
absent — the rule the `F` button already follows. A condition and a log message are **data in the
request**: the adapter compiles the expression and formats the message, so Unluminous's whole cost for two
...
`unluminous_dap::Request::resumes` is where the five spellings of "the program is going again" are written
...
`unluminous-cli debug` is the command line half, thirteen verbs, and the sequence the whole feature is an
...
Unluminous can now observe a program's actual state instead of reasoning about it.
...
Unluminous's, and what Unluminous does about it is what it should — the session ends, `debug status` says so,
...
this appeared at all, and swallowing it is exactly what `unluminous-git`'s rule about git's stderr says
never to do.
...
before `127.0.0.1` on macOS, so an adapter left to its default binds where `unluminous_dap` never dials —
the host is passed on its command line, from `unluminous_dap::SERVER_HOST`, so the two ends cannot
...
`UNLUMINOUS_DAP_TRACE=1` writes every frame in both directions to standard error. It is what found the
...
`task-1692` reported an agent that had to read Unluminous's source, download a debugger and write code to
...
`UnluminousApp::adapter_for` asked of the thing the button acts on, not of whichever tab has focus. So a
...
runs it **on a thread**, the way `unluminous-git` runs git, and takes the binary out of cargo's own
...
it with `Install` and `Copy command`, and `unluminous-cli debug install <adapter>` is the same path — which
...
**`unluminous-cli debug adapters` is the doctor, and it is what an agent runs first.** Every debugger this
...
**Unluminous still fetches nothing.** `tools/get-debug-adapter.sh <node|lldb>` is the macOS and Linux half
...
Unluminous's own rule, and is what makes it impossible to send a request the window manager will throw
...
makes with its own tools is not something Unluminous was told about, so it never appeared. Creating,
...
each costing a walk. Unluminous does not need to watch a tree.
...
Unluminous's windows are one per project and a geometry kept per person would open the second window on
top of the first. `main.rs` reads it before the window is built. `UnluminousApp::note_where_the_window_is`
...
are **dimmed**, which is deliberately the other half of Unluminous's rule. Absent is for a control that can
...
`unluminous-cli explorer new-file`, `new-folder` and `reload` for the agent the ticket is about.
...
**What the pointer is over is a field path, not a word.** `unluminous_core::expressions::at` takes the
...
list of languages inside Unluminous, and the segment walk reads the text through
...
`unluminous-cli debug hover` is the agent's half and takes a **position** — `--line`, `--column`, or the
...
`debug evaluate` returned as `expandable: true`. `unluminous-cli debug set-expression` is the write half.
...
**The shape of the window stopped being a run of `let`s and became a value.** `UnluminousApp::ui` used to
...
the whole width, and the columns come out of what is left*, which is what Unluminous already did and what
...
`terminal_grid_size` ask `UnluminousApp::panel_area`, which answers with the rectangle a panel has — or the
...
`panes.<panel>.order` beside the sizes — and not in the project's `.unluminous`. That is the line
`task-1693` drew: the window's *geometry* belongs to the project, because Unluminous's windows are one per
...
own the layout *and the tab bars*, where Unluminous draws its own furniture at absolute positions measured
...
its panel, because Unluminous has one rail on one edge and a rail that reshuffled itself on every drag
would be a second thing moving while somebody was moving the first.

`unluminous-cli panel list | dock | size | reset` is the command line half, and `status` says which edge
...
visible lines. Every offset in Unluminous is a byte offset into the real file — the caret, the selection,
...
`unluminous_core::folding::regions` reads brackets that span lines, block comments and runs of line
...
exactly the same reason: `insert` and `remove_range` are the only two places in Unluminous that know a
...
if Unluminous ever grew folding — so the gutter is exactly as wide as it was and the text did not move a
...
`Settings` out of reach. They are **absent** for a file that cannot fold, which is Unluminous's rule for a
...
3.8 ms for a whole keystroke with a fold closed. `cargo run --release -p unluminous-app --example
...
`unluminous-cli fold list | toggle | collapse | expand | others` is the command line half, and the MCP
...
**An edit costs the paragraph it changed.** `unluminous_core::relayout` takes the previous layout and
...
`crates/unluminous-app/examples/frame_cost.rs` is how any of this is measured again: it opens a real file
...
picture that was not changing. egui's own README says the opposite is the design; Unluminous draws
...
**`services::frame_trace` is the missing instrument.** `UNLUMINOUS_FRAME_TRACE=<file> unluminous`
writes one line per frame — every phase of `UnluminousApp::ui`, plus `outside`, which is egui's own
...
`unluminous-cli` command**. `instances::is_running` asks the operating system first. On Windows that
...
whole Unluminous window is 223 MB, so the driver is 55% of it and there is no amount of care inside
Unluminous that gets that back. Zed benchmarks at about 222 MB. Do not go looking for it again.
...
| `unluminous-cli` with a stale instance file | 431 ms | **75 ms** |
...
So `unluminous_core::markdown` is the two phases every conforming implementation uses.
...
HTML, and Unluminous has no HTML, no box model and no inline layout, so everything it gave back would have
...
`unluminous-core` holds no plugin registry and must not learn about one, so it asks and the window answers
...
claims, so ```` ```rs ```` and ```` ```rust ```` are one question and Unluminous holds no table of aliases.
...
than having no selection at all. So `Focus` is untouched and one flag, `UnluminousApp::reading_preview`,
...
`unluminous-cli editor preview-select` is the command line half, and `editor preview --json` reports the
...
pane of the side by side view. The preview is not a second renderer — `unluminous_core::markdown` turns
...
is and on how large the picture turns out to be, and `unluminous-core` can know neither — it has no window
...
**The ranges live in `unluminous_core::highlights`, inside the `Document`.** A sparse set, sorted by where
...
alpha, deliberately, because text in Unluminous is always fully opaque; and formatting is inherited by
...
It lives in the document so that `insert` and `remove_range` — the only two places in Unluminous that know
...
**`services::file_marks` is the project's copy**, one `.unluminous/highlights.txt` for the whole project
...
is opened. `UnluminousApp::change_highlights` is the one place that choice is made, so no command has to
think about it.
...
view mode and its own gutter. The same entries are on `View -> Split` and in `unluminous-cli pane`.

**A tab moves into the new pane rather than being copied**, which is the one place Unluminous and IntelliJ
differ and is a decision Jason confirmed on the ticket. IntelliJ's `Split Right` shows the same file
in both splits; Unluminous cannot, because `OpenFiles::open` has always said that a file already open is
...
**What was laid out belongs to the tab, not to the window.** Ten fields moved from `UnluminousApp` onto
...
`UnluminousApp::follow_the_open_file` is one rule for all three: it remembers the path it last revealed and,
...
`unluminous-cli explorer select-open-file` ask for it by hand.
...
names it with it. Two of them needed something Unluminous had never had, and it is the same thing.
...
**`UnluminousApp::close_tab` writes a modified tab before closing it.** Every other editor puts a
three-answer dialog here; Unluminous can give the simpler answer because it saves plain text and nothing
...
**It is one reading, asked twice.** `unluminous_core::imports` gained the forward half of the questions it
...
`unluminous-cli explorer move --dry-run` prints the whole change set and touches nothing, which is the
...
where the list is in order and an index into it is enough. `unluminous-cli explorer move --dry-run`
...
because `unluminous_terminal::Tabs` is a list of sessions and nothing else, and a parallel list of names
...
something in the field, but `unluminous-cli terminal rename --tab 0` can.
...
`unluminous-cli action run` can all ask for — and a right click therefore **shows** the tab first.
`Rename Terminal Tab...` is on the `View` menu as well, because `unluminous-cli action list` is built by
...
`Tabs::move_tab` is the move, `unluminous-cli terminal move` calls it too, and `position` counts the tabs
...
next widget that can take focus. Every control Unluminous draws with `Sense::click` can. The editing area,
...
That was reported as Unluminous crashing while somebody typed, and there was nothing to find: no panic, no
...
Unluminous draws only when something happens, and everything that happens on another thread — a command
...
to revert to if a bug turns up. Unluminous runs the loop once and then the process ends.
...
boxes. Pressing play spawns a `unluminous_terminal::Session` with the program in place of the shell, so
the output is a real terminal and stopping is killing a process Unluminous owns.
...
**They live in `.unluminous/run-configurations.conf`**, numbered `run.N.*` the way `files.panes` numbers
...
leaving it to each caller did not survive first contact: `unluminous-cli terminal show` set its own flag,
...
and `Run::ended` is where the answer is kept the moment it arrives: a program Unluminous killed has no
...
and `node hello.js` two runs in three. `UnluminousApp::run_grid_size` therefore works the size out from
...
{file}`. `run.project` names a detector **built into Unluminous**, checked against
`plugins::PROJECT_RUNNERS` exactly as `language.renders` is checked against `RENDERERS`; `cargo`
reads `Cargo.toml` and `npm` reads a `package.json`'s scripts, both in Unluminous's own code, so the most
...
`unluminous-cli run` is the whole feature from the command line, and `run output` is the one to notice:
...
commands. Four rules came out of it, and a change to the terminal or to what Unluminous remembers has to
keep all four.

**A path Unluminous writes down or hands to a program is plain.** `std::fs::canonicalize` on Windows gives
back a **verbatim** path — `\\?\C:\jason\dev\unluminous` — and every Rust file call takes one happily, so
nothing inside Unluminous notices while it travels: into `recent.txt`, onto the explorer's root, and from
...
`unluminous_terminal::paths::plain` takes the prefix off. `Store::remember_project` calls it so one is never
...
every Windows there is, so reading it meant Unluminous's terminal never held the commands in a person's
...
**With no path on the command line, where Unluminous starts depends on how it was started.** The current
directory is the honest answer when somebody typed `unluminous` in a terminal and no answer at all from a
desktop shortcut, where it is only wherever the shortcut points. `unluminous_app::starting_folder` reopens
...
identity for this repository in particular. A push from Unluminous has to be the same push you get in the
...
detached HEAD and a missing upstream all explain themselves better than Unluminous could.

**Every command goes through `unluminous_git::Worker`**, which runs one at a time on a thread. Not because
...
drawn for a `.txt` one. Unluminous saves plain text and carries no formatting to disk, so everything behind
...
window, the way IntelliJ has one editor font. `UnluminousApp::set_the_font_everywhere` is what puts it
...
the old one until Unluminous was restarted.
...
**What is remembered is the text, not the offset.** `unluminous_core::Anchor` is where the line the point
...
`UnluminousApp::zoom_over_a_panel` is what the explorer, the three tiles and each contributed pane call.
...
`unluminous-cli panel zoom <panel> [factor]` is the agent's half, and it answers with whichever of the two
...
showing into the project's own `.unluminous` on every frame, so a window closed with a pane maximised would have
...
**What both halves agree about is the text**, so `unluminous_core::markdown::Preview` carries a fourth
...
`unluminous_core::scroll_sync` is the crossing, two pure functions and no state: which **paragraph** is at
...
`follow_the_other_half` is split out because **the command line needed it**: `unluminous-cli editor scroll`
...
So the strip does what every component in Unluminous does — it reports what happened and decides nothing.
...
`OpenFiles::drag_tab` does the move and is what `unluminous-cli tab move` calls too. Its one subtlety:
...
**It is two calls rather than one**, which is the only place a component in Unluminous is. `grab` takes the
...
back to its usual size are decided in that one file, so every divider in Unluminous behaves the same way.
...
- Its size belongs in `settings::Panes`, which is written to the settings file, so the pane is where it was
  left next time Unluminous starts. Give it a smallest and a largest size and clamp both when reading the file
  and when dragging.

## A project remembers what was open in it, beside the project

`services::project_state` writes a `.unluminous` folder **inside the project** — beside `.idea` and
...
**Only the released binary reads or writes it.** `UnluminousApp::restore_project` is called from `main.rs`
and by nothing else, exactly as `load_settings` is. A test must not touch a person's files, and a
`.unluminous` written into a screenshot test's own sample project would change what the explorer draws in
...
`unluminous-cli` drives a **running** Unluminous: `unluminous-cli tab open README.md`, `unluminous-cli terminal send git
status`, `unluminous-cli settings set appearance.font.size 20`, `unluminous-cli window screenshot after.png`.
...
describes a command that no longer exists. `cargo run -p unluminous-cli --example reference` writes it. A
...
catalogue sends — and an `arguments` object is written by hand by anything that is not `unluminous-cli`,
...
Unluminous watches nothing and a tab is owned by its `Document`, which is right while Unluminous is the only
...
`UnluminousApp::run_cli` is to the command line what `run_action` is to the menus: **the one place a
...
`unluminous --control off` closes it. `tasks/unluminous-cli-tdd.md` records what was weighed;
...
code Unluminous does not own: `ContextImpl::request_repaint_after` calls the backend's callback only `if
...
**How well it reads.** `unluminous-cli/agent-assessment/` measures it rather than assuming it: the local
...
Unluminous has a Model Context Protocol server, so a client that speaks it — Claude Code, Codex, anything
else — is handed the commands as tools rather than being handed `docs/commands.md` and told to shell
out. `unluminous_cli::mcp` is all of it, in the CLI crate rather than the window, because that is the way
the dependency already points: the window can host it without the client ever learning what a window
is. `task-1679` is the design, `unluminous-cli/docs/mcp.md` is what a person needs.

**The tools are generated from `catalogue.rs`.** This is the fourth rule in the section above and it
is the reason the module exists: a command added to Unluminous is a tool the day it is added, with its
...
first measured it; the ratio is what matters.) `unluminous-cli mcp tools --count` prints both figures
...
`UnluminousApp::run_action` is the only place an action turns into a change. There are two menu bars, the one
macOS draws along the top of the screen and the one Unluminous draws inside its own title bar on Windows, and
...
once and writes `unluminous.ico`, `unluminous.icns` and an iconset; `installer/windows` compiles an Inno Setup
script into a single setup exe; `installer/macos` builds `Unluminous.app` and a disk image. Each has a build
...
The one place it does reach in is `crates/unluminous-app/build.rs`, which puts the icon and a version block
inside `unluminous.exe`. That has to be inside the executable — Windows reads the taskbar icon, the Alt-Tab
...
still produces a working, if unlabelled, `unluminous.exe`.
...
stamped by `crates/unluminous-app/build.rs` while the binary is compiled — the local time of the machine
...
fingerprint, so a script that restamped on every invocation would recompile `unluminous-app` and relink
...
of `main`. `cargo run --release -p unluminous-app --example startup_cost` is its other half, for the
...
session with no shell behind it, through `UnluminousApp::new_detached_terminal_tab`, because when a real shell
...
not a fault in Unluminous (`task-1654` — the file's own comments carry the detail):
...
Tests must not read or write the settings of the person running them. `UnluminousApp::new` reads nothing; the
...
the line an Unluminous task ends on and a person about to type into their own machine again should not have
...
**The Windows key is not Unluminous's to strand, and that is worth knowing before hunting for it here.**
...
Unluminous window is a second process.
...
`language.renders = mermaid`, naming a renderer that is **built into Unluminous**. Nothing is loaded from
...
breaks three of the rules in `unluminous_core::syntax`: **a hyphen is a letter** — nearly every property
...
The body of a raw text element is another language, and `unluminous-core` says where it is and what it
...
- `README.md` — what Unluminous is and how to run it.
- `documentation/overview.md` — what Unluminous looks like: a capture of each part of the window, over a real desktop.
...
- `tasks/unluminous-technical-design-document.md` — the editor: the options that were considered, what was
  chosen and why, and what is deliberately not included.
- `tasks/unluminous-terminal-tdd.md` — the same for the terminal.
...
- `tasks/task-1658-window-improvements-tdd.md` — the window's own resize grips, the rail of pane
  buttons, the `.unluminous` folder a project remembers itself in, the tools moving into the title bar,
...
- `tasks/task-1670-terminal-tdd.md` — the terminal that opened in `C:\Windows` running the wrong
  shell: the verbatim Windows path and where it came from, why `COMSPEC` is not the shell, when Unluminous
...
- `tasks/task-1677-autocomplete-tdd.md` — auto-complete: what each tier of editor does and which of
  ...
  implementation of it; `cargo run --release -p unluminous-app --example completion_cost` is how its one
...
- `tasks/task-1680-import-completion-tdd.md` — completing an import: the two shapes a module is
  ...
  languages inside Unluminous would have cost instead, what auto-import would take, and the fifty-one
  scenario battery.
...
- `tasks/task-1675-code-editing-tdd.md` — go to definition, find all references and rename: the
  ...
  `task-1676` is the implementation of it; `cargo run --release -p unluminous-app --example symbol_cost`
...
- `tasks/task-1767-agent-chat-tdd.md` — the chat pane: how the picture it is measured against was
  ...
  on a platform with no keychain, what a tool call is and why it is Unluminous's own catalogue, and the
  eight things deliberately left out.
...
- `tasks/task-1683-run-configurations-tdd.md` — run configurations: what IntelliJ's model, widget
  and Run tool window each are, why a configuration in Unluminous is one named command rather than a
...
- `tasks/unluminous-mermaid-plugin-tdd.md` — Mermaid: the four ways of drawing it that were weighed and why
  Unluminous writes its own, what each of the twenty types becomes on the screen, which ten are named
...
- `tasks/unluminous-cli-tdd.md` — the command line: the transports that were weighed, the command surface,
...
- `unluminous-cli/docs/commands.md` — **the reference, written to be handed to an AI agent whole.**
- `unluminous-cli/docs/protocol.md` — the socket underneath, for a client in another language.
- `unluminous-cli/agent-assessment/qwen-38-27B-assessment.md` — how well a local model does with it,
...
- `tools/agent-study/README.md` — the harness that watches an agent drive a real window, and
  ...
  Unluminous's own commands, the debugger answer it derived rather than read, and the nine tickets
  that came out. Add a scenario when you add a feature.
- `tasks/unluminous-installer-tdd.md` — how Unluminous is delivered: the icon, the Windows installer and the
