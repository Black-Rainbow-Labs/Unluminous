...
```
releases/
  UnluminousSetup-0.2.0-x64.exe   Windows: the Inno Setup installer, uploaded to the v0.2.0 release
  unluminous-0.1.0.dmg            macOS: the application and an alias to /Applications
```
...
for something a person downloads. (`unluminous-0.1.0.dmg` predates that decision and is still tracked.)
...
**Which build am I running?** `Unluminous -> About Unluminous` says, and so does `unluminous-cli status --json`:
both carry the version and the date the binary was built.

## What a person who receives one of these sees

**macOS.** `unluminous-0.1.0.dmg` is signed with a Developer ID and notarised, with the ticket stapled to the
...
takes a right click on `Unluminous.app` and *Open*, once. Which of the two a file is can be read from the file
itself:

```bash
spctl --assess --type open --context context:primary-signature --verbose=2 releases/unluminous-0.1.0.dmg
xcrun stapler validate releases/unluminous-0.1.0.dmg
```
