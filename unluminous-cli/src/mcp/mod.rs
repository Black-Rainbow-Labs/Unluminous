//! The Model Context Protocol server: how an AI agent discovers and drives Unluminous.
//!
//! `task-1661` gave Unluminous a command line and then measured an agent using it — the local Qwen 3.8
...
//! exactly the request `unluminous-cli` would have sent, down the same loopback socket with the same
//! token, so `UnluminousApp::run_cli` stays the one place a command becomes a change.
//!
//! ## Two transports
//!
//! [`stdio`] is what an agent launches: `unluminous-cli mcp serve`, one JSON message a line over the
...
//! rather have a URL — and it is what a running Unluminous hosts when `mcp.enabled` is on. It is off by
...
/// The port an Unluminous hosting the HTTP endpoint listens on unless somebody chose another.
///
...
pub enum Transport {
    /// The agent launches `unluminous-cli mcp serve` and talks down its pipes. No port.
    #[default]
    Stdio,
