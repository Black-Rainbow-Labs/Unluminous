//! Where a running Unluminous says how to reach it, and how the client finds one.
//!
//! An Unluminous that is listening writes one small file into `<settings folder>/instances`, named after
...
//! ## Stale files
//!
//! An Unluminous that is killed rather than closed leaves its file behind. Nothing sweeps them on a
...
//! `unluminous-cli` command**, and **414 ms of the next window's startup**, which was a third of it.
//! Unluminous is killed rather than closed routinely — the task manager, a crash, a script — so this was
//! not a rare path.
//!
...
///
/// Inside the folder Unluminous already keeps its settings in, so there is one place per person that
/// belongs to Unluminous rather than two. `UNLUMINOUS_INSTANCES` names another folder, which is what the
/// tests use so that they never see the person's real windows.
pub fn folder() -> PathBuf {
    if let Some(named) = std::env::var_os("UNLUMINOUS_INSTANCES") {
...
///
/// This is deliberately the same rule as `unluminous_app::services::store::settings_folder`, and there
/// is a test in `unluminous-app` that fails if the two ever come to disagree. It is written twice
...
///
/// Nothing is checked here beyond the file parsing. Whether the Unluminous it names is still running is
...
/// Remove an instance's file, which an Unluminous does as it stops and the client does when it finds one
...
mod tests {
    ...
    fn the_newest_instance_is_listed_first() {
        let folder = std::env::temp_dir().join("unluminous-cli-instances-order");
    ...
    fn a_file_that_is_not_an_instance_file_is_ignored() {
        let folder = std::env::temp_dir().join("unluminous-cli-instances-other");
        std::fs::remove_dir_all(&folder).ok();
        std::fs::create_dir_all(&folder).expect("make the folder");
