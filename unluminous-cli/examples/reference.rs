//! Write the reference half of `unluminous-cli/docs/commands.md` from the catalogue.
//!
//! Run it with `cargo run -p unluminous-cli --example reference`.
//!
...
use unluminous_cli::catalogue::{self, Command};
...
fn section(command: &Command) -> String {
    ...
    out.push_str("```\n\n");
    if command.local {
        out.push_str("Answered by the CLI itself; no Unluminous needs to be running.\n\n");
    }
    out
}
