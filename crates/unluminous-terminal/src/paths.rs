...
//!
//! `std::fs::canonicalize` on Windows does not give back `C:\jason\dev\unluminous`. It gives back
//! `\\?\C:\jason\dev\unluminous` — a **verbatim** path, the form that lets a path be longer than 260
//! characters and that is handed to the file system with none of the parsing every other path goes
//! through. Rust's own file calls take it happily, so nothing inside Unluminous notices, and it travels: into
...
//! `unluminous-cli terminal` all can, and a list of the places that have to remember to strip it is a list
...
/// `C:\project\src/report.rs` — a path with both separators in it. Nothing inside Unluminous notices,
...
///
/// It is `Path`'s own normalisation rather than a text substitution, so the path Unluminous hands over is
/// the path Unluminous compares. On a platform where `\` is an ordinary character in a filename nothing
...
mod tests {
    ...
    /// handed to another program and a path compared inside Unluminous cannot come apart.
    #[test]
    ...
    fn stripping_twice_is_stripping_once() {
        let once = plain(Path::new(r"\\?\C:\jason\dev\unluminous"));
