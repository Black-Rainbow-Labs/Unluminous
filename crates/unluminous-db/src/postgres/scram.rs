...
/// The fewest rounds of PBKDF2 Unluminous will derive a password with.
///
    ...
    pub fn respond(&mut self, server_first: &str) -> Answer<String> {
        ...
        if !nonce.starts_with(&self.client_nonce) || nonce.len() <= self.client_nonce.len() {
            return Err(Failure::said(
                "the server's SCRAM nonce is not the one Unluminous sent with the server's own added to \
        ...
        if iterations < LEAST_ROUNDS {
            ...
            return Err(Failure::said(format!(
                "the server asked for {iterations} rounds of PBKDF2, and Unluminous will not derive a \
...
mod tests {
    ...
    fn a_server_nonce_that_does_not_extend_the_clients_is_refused() {
        ...
        let refused = exchange
            ...
            .expect_err("refused");
        assert!(refused.message.contains("is not the one Unluminous sent"), "{refused}");
    }
