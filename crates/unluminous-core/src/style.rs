...
//! document of the size Unluminous targets never holds enough spans for the difference to matter and a
//! vector is much easier to test.

use std::ops::Range;

/// A colour with no alpha. Text in Unluminous is always fully opaque, so there is no alpha to store; see
