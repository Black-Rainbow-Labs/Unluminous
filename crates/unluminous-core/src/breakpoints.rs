...
/// adapter in `SourceBreakpoint`, so Unluminous's whole cost for two of the reference editor's features is these two
...
impl Breakpoints {
    ...
    ///
    /// What a file that changed on the disk underneath Unluminous needs, and what reading
    /// `.unluminous/breakpoints.conf` needs: the offsets were written against bytes that may no longer be
...
mod tests {
    ...
    /// A file rewritten outside Unluminous gives a misplaced dot rather than a panic in the layout
