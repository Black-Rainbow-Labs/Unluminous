...
//! chose. `unluminous-dap` is arranged the same way and for the same reason: its tests run against
...
//! serialise. That is the choice `unluminous-cli/src/protocol.rs` and `unluminous-dap` already made.
//!
...
//! the better one. Unluminous holds **no key at all**; the agent brings its own tools, its own sandbox and
...
//! one part of Unluminous that opens a socket or starts a program.
