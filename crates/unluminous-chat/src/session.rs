...
//! tools, one that fails after saying something. `unluminous_dap::session` is arranged the same way and
//! for the same reason.
//!
...
impl Session {
    ...
    pub fn reply(&mut self, reply: Reply) {
        match reply {
            ...
            // **A tool the agent ran itself.** Unluminous did not run it and has nothing to send back: it
    ...
    /// running for ever and the turn stopped with no way out of it. A position is Unluminous's own and
    /// cannot collide.
