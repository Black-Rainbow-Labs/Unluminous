...
//! `api.anthropic.com` in every way that matters here: there is **no key for Unluminous to hold at all**,
//! the agent brings its own tools and its own permission model rather than being handed Unluminous's, it
...
//! ## Where a key lives, which is nowhere Unluminous writes
//!
...
//! keychain** in Unluminous — on Windows it answers `None` and refuses to write.
//!
//! So a provider names an **environment variable** and Unluminous reads it at the moment a request is
//! sent. `ANTHROPIC_API_KEY` is the name every tool on this machine already uses and the one
//! `claude` itself reads, so on the platform Unluminous is developed on there is a way to have a key that
...
///
/// The sixth registry of this shape in Unluminous, after the renderers, the project detectors, the
...
impl Wire {
    ...
    /// catalogue from Unluminous, and its conversation is continued by its own session id rather than by
...
pub struct Provider {
    ...
    /// API and the llama.cpp on this machine, and a base URL that Unluminous appended a path to would be
    /// an Unluminous that decided what somebody's gateway looks like.
    pub url: String,
    pub model: String,
    ...
    pub fn defaults() -> Vec<Provider> {
        vec![
            Provider {
                ...
                // tools, it reads the project's own `CLAUDE.md`, and Unluminous holds no key for it.
                wire: Wire::ClaudeCli,
...
/// page as `.\agent.exe`, or found through a relative entry in `PATH` — is checked against Unluminous's
...
/// Windows answers with a verbatim `\\?\` path — the exact thing `unluminous_terminal::paths::plain`
...
mod tests {
    ...
    fn the_two_rows_the_ticket_names_are_command_lines_and_neither_wants_a_key() {
        ...
        // the better half of the feature: there is nothing for Unluminous to store, read or redact.
