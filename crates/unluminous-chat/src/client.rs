...
//!
//! Arranged as `unluminous_git::Worker`, the terminal's reader loop and `services::text_search` already
//! are: a thread, a channel and a waker. The window must not stop drawing while a model is thinking,
//! and on a long answer that is a minute — which, as `unluminous-git`'s own comment about a fetch says,
...
/// macOS, so Unluminous trusts what the machine trusts.
///
...
/// refused the pane. `PlatformVerifier` is the whole of the fix, and it is `unluminous-git`'s argument for
...
pub fn run(
    ...
    // verbatim, which is `unluminous-git`'s rule and the right one; but an endpoint that echoes the
    // request back — a debugging proxy, a misconfigured gateway, or one written to do it — would put
    // the key in the pane and then in the transcript on disk, which is the one thing Unluminous promises
    ...
    let config = ureq::Agent::config_builder()
        ...
        // server had to say — which is what `unluminous-git`'s rule about never inventing an error
...
mod tests {
    ...
    ///
    /// `unluminous-dap`'s scripted adapters with a socket instead of a pipe, and it is what makes "the
