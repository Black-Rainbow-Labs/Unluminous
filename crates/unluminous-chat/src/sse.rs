...
//! yields whole events, which is `unluminous_dap::codec`'s arrangement for `Content-Length` framing and
...
mod tests {
    ...
    fn a_stream_split_at_every_boundary_reads_the_same() {
        ...
        // could enumerate the ways it might. `unluminous_dap`'s framing has the same test.
