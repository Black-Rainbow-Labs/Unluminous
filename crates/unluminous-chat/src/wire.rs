...
//! [`Reply`]s.
//!
//! This is the only file in Unluminous that knows what either API looks like. Everything above it — the
...
//!
//! `unluminous-git` shells out to the real git and shows what git said, because a rejected push explains
//! itself better than Unluminous could. The same holds here for a 401, a 429, a model that does not
...
/// `unluminous_core::mermaid::Scene`'s items: values with no behaviour, so a session can be driven by
...
pub enum Reply {
    ...
    /// the agent has one whether or not Unluminous waited for the answer.
    Session(String),
    /// More words.
    Text(String),
    ...
    /// sandbox and their own permission model, so Unluminous does not run anything on their behalf — what
    /// it does with a tool call is *show* it, and this is the other half of showing it. An HTTP
    /// endpoint never sends one, because there the model asks and Unluminous answers.
    ToolAnswer {
        id: String,
        answer: String,
        failed: bool,
...
/// One tool Unluminous offers the model.
///
/// Built from `unluminous-cli`'s catalogue rather than written out, which is the rule
/// `unluminous_cli::mcp::tools` already keeps: a command added to Unluminous is a tool the day it is added.
...
fn openai_request(
    ...
    for message in &chat.messages {
        // A turn that failed leaves a message holding only Unluminous's own note about what went wrong.
...
fn responses_request(
    ...
    let mut body = json!({
        ...
        "input": input,
        "stream": stream,
        // Off, because Unluminous has nothing to do with a conversation the server is keeping: the
...
mod tests {
    ...
    fn the_openai_request_is_what_that_api_documents() {
        let provider = openai();
        let body = request(&provider, &a_chat(), "You are in Unluminous.", &[], true);
        ...
        assert_eq!(body["messages"][0]["content"], "You are in Unluminous.");
    ...
    fn the_anthropic_request_puts_the_system_prompt_in_a_field_of_its_own() {
        ...
        let body = request(&provider, &a_chat(), "You are in Unluminous.", &[], true);
        assert_eq!(body["system"], "You are in Unluminous.");
        assert_eq!(
...
event: error\ndata: {\"type\":\"error\",\"error\":{\"type\":\"overloaded_error\",\"message\":\"Overloaded\"}}\n\n",
    ...
    fn a_stream_cut_off_mid_answer_is_a_failure_rather_than_an_ordinary_stop() {
        ...
        let stream = concat!(
            ...
            "data: {\"choices\":[{\"delta\":{\"tool_calls\":[{\"index\":0,\"id\":\"c\",\"function\":{\"name\":\"unluminous_editor\",\"arguments\":\"{\\\"pa\"}}]}}]}\n\n",
        );
