...
use unluminous_chat::model::{Conversation, Message, Role, ToolCall};
use unluminous_chat::provider::{Provider, Wire};
use unluminous_chat::sse;
use unluminous_chat::wire::{self, Decoder, Tool};
use unluminous_chat::Reply;
...
fn the_responses_request_is_a_list_of_items_rather_than_a_list_of_messages() {
    ...
    answering.tools.push(ToolCall::new("call_a", "unluminous_git", "{}"));
    chat.push(answering);
    let mut results = Message::new(3, Role::Tool);
    let mut answered = ToolCall::new("call_a", "unluminous_git", "{}");
    ...
    assert_eq!(with_tools["tools"][0]["name"], "unluminous_git");
