...
//! `crates/unluminous-app/tests/snapshots`. When a run differs from the accepted image the harness also
...
//!
//! Run `UPDATE_SNAPSHOTS=1 cargo test -p unluminous-app` to accept new images.
...
fn open_text_options(harness: &mut Harness<'static, UnluminousApp>) {
...
fn collapse(harness: &mut Harness<'static, UnluminousApp>) {
...
fn showing(harness: &Harness<'static, UnluminousApp>, key: &str) -> bool {
    harness
        .state()
        .plugin_ui
        .slot_of(key)
...
fn pump(harness: &mut Harness<'static, UnluminousApp>) {
...
fn git_harness(name: &str) -> Harness<'static, UnluminousApp> {
...
fn drag(harness: &mut Harness<'static, UnluminousApp>, from: egui::Pos2, to: egui::Pos2) {
...
fn open_about(harness: &mut Harness<'static, UnluminousApp>) {
    harness.state_mut().menu_placement = MenuPlacement::InWindow;
    harness.run();
...
fn open_settings(harness: &mut Harness<'static, UnluminousApp>) {
    harness.state_mut().menu_placement = MenuPlacement::InWindow;
    harness.run();
...
fn a_folder_of_long_files() -> std::path::PathBuf {
    let root = std::env::temp_dir().join("unluminous-zoom-folder");
...
fn double_click_at(harness: &mut Harness<'static, UnluminousApp>, at: egui::Pos2) {
    let modifiers = Modifiers::default();
    harness.input_mut().events.push(egui::Event::PointerMoved(at));
    for _ in 0..2 {
        for pressed in [true, false] {
...
fn with_terminal(text: &str, rows: usize, columns: usize) -> Harness<'static, UnluminousApp> {
    let mut harness = harness(text);
...
fn nudge(harness: &mut Harness<'static, UnluminousApp>) {
    for _ in 0..4 {
        pump(harness);
    }
}
...
fn double_click(harness: &mut Harness<'static, UnluminousApp>, label: &str) {
...
fn did(harness: &mut Harness<'static, UnluminousApp>, line: &str) -> serde_json::Value {
...
fn refused(harness: &mut Harness<'static, UnluminousApp>, line: &str) -> String {
    let reply = run(harness, line);
...
fn a_folder_with_a_long_source_file(name: &str) -> std::path::PathBuf {
    let root = std::env::temp_dir().join("unluminous-performance").join(name);
    std::fs::create_dir_all(&root).expect("make the folder");
...
fn right_click_at(harness: &mut Harness<'static, UnluminousApp>, at: egui::Pos2) {
...
fn choose(harness: &mut Harness<'static, UnluminousApp>, action: Action) {
    let ctx = harness.ctx.clone();
    harness.state_mut().run_action(action, &ctx);
    harness.run();
}
...
// behaviour underneath them is tested with no window in `unluminous_core::symbols`, `app::symbols` and
...
fn code_folder() -> std::path::PathBuf {
    ...
    FOLDER
        .get_or_init(|| {
            let root = std::env::temp_dir().join("unluminous-screenshot-code");
...
// `unluminous_core::imports`, `services::imports` and `app::completion`.
...
fn import_folder() -> std::path::PathBuf {
    ...
    FOLDER
        .get_or_init(|| {
            let root = std::env::temp_dir().join("unluminous-screenshot-imports");
...
// underneath is tested with no window in `unluminous_core::completion` and `app::completion`; what is
...
fn completion_folder() -> std::path::PathBuf {
    ...
    FOLDER
        .get_or_init(|| {
            let root = std::env::temp_dir().join("unluminous-screenshot-completion");
...
fn completions(harness: &Harness<'static, UnluminousApp>) -> Vec<String> {
    harness
        .state()
        .completion()
...
fn scratch_folder(name: &str) -> std::path::PathBuf {
    let root = std::env::temp_dir().join(format!("unluminous-1681-{name}"));
    std::fs::remove_dir_all(&root).ok();
...
fn folding_folder(name: &str) -> std::path::PathBuf {
    let root = std::env::temp_dir().join("unluminous-screenshot-folding").join(name);
    std::fs::remove_dir_all(&root).ok();
    std::fs::create_dir_all(&root).expect("make the folder");
    std::fs::write(
        root.join("source.rs"),
...
fn folding_harness(name: &str) -> Harness<'static, UnluminousApp> {
...
fn debug_folder(name: &str) -> std::path::PathBuf {
    let root = std::env::temp_dir().join("unluminous-screenshot-debug").join(name);
    std::fs::remove_dir_all(&root).ok();
    std::fs::create_dir_all(&root).expect("make the folder");
    std::fs::write(
        root.join("main.rs"),
...
fn javascript_folder() -> std::path::PathBuf {
    ...
    FOLDER
        .get_or_init(|| {
            let root = std::env::temp_dir().join("unluminous-screenshot-javascript");
            std::fs::create_dir_all(&root).expect("make the folder");
...
fn javascript_harness() -> Harness<'static, UnluminousApp> {
...
fn asked(harness: &mut Harness<'static, UnluminousApp>) -> Vec<serde_json::Value> {
...
/// only happens there.
fn type_and_paint(harness: &mut Harness<'static, UnluminousApp>, text: &str) {
    for letter in text.chars() {
        if letter == '\n' {
            harness.key_press(egui::Key::Enter);
        } else {
...
#[test]
fn the_board_contributes_a_tab_and_no_pane() {
    use unluminous_app::app::dock::Panel;
    let mut harness = harness("");
    ...
    harness.run();
    assert!(
        harness.state().files.active().name().contains("Agent-Tasks"),
        "the menu entry opened the board as a tab, and the tab is showing"
    );
}
...
fn a_pane_or_a_command_nobody_has_is_refused_with_what_there_is() {
    ...
    // `task-28`: the board is a tab and contributes no pane, so asking for its pane by its own name is
    // refused in exactly the same way as asking for one that never existed.
...
#[test]
fn the_plugin_opens_a_tab_in_the_editing_area_beside_the_file_tabs() {
    let mut harness = harness("Some prose.");
    assert_eq!(harness.state().files.len(), 1, "the file it opened with");
    let opened = did(&mut harness, "plugins tab agent-tasks/board --open");
    assert_eq!(opened["open"], true);
    harness.run();
    assert_eq!(harness.state().files.len(), 2, "the file, and the board beside it");
    ...
    // Opening it again shows the one that is open rather than opening a second.
...
fn the_plugins_menu_is_after_unluminouss_own_six_and_its_entries_run_through_one_path() {
    ...
    assert!(labels.contains(&"Open Board"), "{labels:?}");
    ...
    let name = names
        .iter()
        .find(|name| name.ends_with(":open-tab"))
        .expect("the Open Board entry names itself");
    did(&mut harness, &format!("action run {name}"));
    harness.run();
    assert!(harness.state().plugin_ui.is_open("agent-tasks"), "the menu entry opened the board");
}
...
fn switching_the_plugin_off_withdraws_every_contribution_in_the_same_frame() {
    ...
    // Unluminous opening a blank one to avoid showing none at all.
    let mut harness = harness("Some prose.");
    did(&mut harness, "plugins tab agent-tasks/board --open");
    harness.run();
...
#[test]
fn reloading_the_plugins_closes_a_tab_whose_contribution_has_gone() {
    // A manifest edited by hand can take a tab away, and a tab whose plugin no longer offers it would draw
    // nothing and could not be told what it was. So a reload closes it, for the reason switching a plugin
    // off does.
    let mut harness = harness("Some prose.");
    did(&mut harness, "plugins tab agent-tasks/board --open");
    harness.run();
    ...
    did(&mut harness, "plugins disable agent-tasks");
    harness.run();
    assert_eq!(harness.state().files.len(), 1, "the board's tab went with the plugin");
...
const NO_SUCH_AGENT: &str = "unluminous-no-such-agent-anywhere";
