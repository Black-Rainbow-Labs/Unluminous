//! Tests that start a real agent on a real board and grade what happened against Unluminous's own state.
//!
...
//!
//! This is the bargain `tools/agent-study` makes, applied to the board. What is asserted is **a state Unluminous
...
//!
//! ```text
//! cargo test -p unluminous-app --test agent_board -- --ignored --test-threads=1
//! ```
//!
...
//! gateway `~/.zshrc` exports. `UNLUMINOUS_TEST_MODEL` names the model, and without it the ticket names none and the
...
use unluminous_app::services::agent_tasks::model::{Assignee, Author, Status};
use unluminous_app::services::agent_tasks::store::{NewTask, Store};
use unluminous_app::services::agent_tasks::{AgentTasks, Configuration};
use unluminous_app::services::plugin_ui::{Context, UiProvider};
...
/// everything was working. `UNLUMINOUS_TEST_MODEL` names a quicker model when somebody wants a quicker run, and the
...
/// The folder the agent is allowed to work in, named by `UNLUMINOUS_TEST_PROJECT`, or `None` to skip.
///
...
/// `tasks/unluminous-issues-tdd.md` §16.4 records that as a finding about the plugin.
///
...
///
/// So the operator names the folder. `UNLUMINOUS_TEST_PROJECT` is a folder they have already opened Claude Code in, and
    ...
    let Some(named) = named else {
        eprintln!(
            "skipped: set UNLUMINOUS_TEST_PROJECT to a folder you have already opened Claude Code in, which is how \
             ...
             begins. See tasks/unluminous-issues-tdd.md section 16.4."
        );
        return None;
    };
...
fn there_is_a_key() -> bool {
    let found = unluminous_app::services::agent_tasks::the_key().is_some();
    if !found {
        eprintln!(
...
fn now() -> String {
    unluminous_app::services::agent_tasks::clock::now()
}
...
fn the_board_shows_the_agents_work_while_it_is_happening() {
    ...
    let said = format!("unluminous-agent-board-said-{}.txt", std::process::id());
...
fn a_claim_whose_worker_is_gone_is_struck_and_the_board_says_why() {
    ...
    let an_hour_ago = unluminous_app::services::agent_tasks::clock::from_unix(
        unluminous_app::services::agent_tasks::clock::to_unix(&now()).expect("now") - 60 * 60,
    );
...
fn the_board_on_this_machine_opens_and_can_be_claimed_on() {
    ...
    let folder = std::env::temp_dir().join(format!("unluminous-real-board-{}", std::process::id()));
    let _ = std::fs::remove_dir_all(&folder);
    std::fs::create_dir_all(&folder).expect("a folder");
    let copy = folder.join("board.sqlite3");
    // Through Unluminous's own copy, which is `VACUUM INTO` rather than a file copy: this board is in write ahead
