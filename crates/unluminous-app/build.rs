//! Two things that have to happen while `unluminous.exe` is being built: the icon and version block go
...
//!
//! The icon is `installer/icon/unluminous.ico`, which is drawn by `installer/icon` and committed, so a
...
//! rather than an error: the build still produces a working `unluminous.exe`, just an unlabelled one, and
...
//! handed to the compiler as `UNLUMINOUS_BUILD_DATE`, which `build_info::BUILD_DATE` reads.
//!
//! **When this script reruns is the whole design.** The value it emits is part of the crate's
//! fingerprint, so a script that restamped on every invocation would recompile `unluminous-app` and
...
fn main() {
    ...
    println!("cargo:rerun-if-changed=../../unluminous-cli");
    println!("cargo:rerun-if-env-changed=UNLUMINOUS_BUILD_DATE");
    println!("cargo:rustc-env=UNLUMINOUS_BUILD_DATE={}", build_date());
...
fn build_date() -> String {
    if let Ok(given) = std::env::var("UNLUMINOUS_BUILD_DATE") {
...
///
/// Unluminous has no dates library and does not want one: `unluminous_git::blame` computes a civil date from a
...
/// `unluminous-git` makes when it runs `git` rather than reimplementing it. `None` when the command
...
/// date is Howard Hinnant's `civil_from_days`, which is the same arithmetic `unluminous_git::blame` uses
...
fn embed_windows_resources() {
    // From `crates/unluminous-app` up to the repository, then down to the drawn icon.
    const ICON: &str = "../../installer/icon/unluminous.ico";
    println!("cargo:rerun-if-changed={ICON}");

    if !std::path::Path::new(ICON).exists() {
        println!("cargo:warning=unluminous.exe has no icon: {ICON} is missing. Run `cargo run --release --manifest-path installer/icon/Cargo.toml`.");
        return;
    }
    ...
    resources
        .set_icon(ICON)
        .set("ProductName", "Unluminous")
        .set("FileDescription", "Unluminous")
        .set("CompanyName", "Jason McAffee")
        .set("LegalCopyright", "Licensed under MIT or Apache-2.0")
        .set("InternalName", "unluminous.exe")
        .set("OriginalFilename", "unluminous.exe");

    if let Err(problem) = resources.compile() {
        println!("cargo:warning=unluminous.exe has no icon or version block: {problem}");
    }
}
