...
//! they agree.
//!
//! Neither knows what it is for. The window holds them and `UnluminousApp::run_action` decides what an
...
fn show_result(ui: &egui::Ui, area: Rect, dialog: &EvaluateDialog, paused: bool) {
    let (text, tint) = match (&dialog.result, dialog.asking, paused) {
        ...
        // better than Unluminous could, which is the rule `unluminous-git` keeps about git's standard error.
