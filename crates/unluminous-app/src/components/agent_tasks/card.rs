...
/// in **this** window, and whether Start could do anything. Unluminous's rule is that a control which cannot
...
pub fn show(
    ...
    // A card under the pointer wears the pill every list in Unluminous draws for its chosen row, rather than a
    ...
    if look.chrome.is_recording() {
        ...
        // nothing — and it is the same pill every list in Unluminous draws for the row it is on.
    ...
    if let Some(key) = &task.jira_key {
        if chip + 60.0 < right {
            ...
            // that could open a browser. Unluminous has no way to open a URL: `services::launcher` opens a folder in
