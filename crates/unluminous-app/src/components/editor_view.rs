...
//!
//! Nothing here decides what an edit means. Every key press becomes a `unluminous_core::Command` and the
...
/// the window decides" rule every component in Unluminous follows.
///
    ...
    for event in events {
        match event {
            ...
            egui::Event::Key { key, pressed: true, modifiers, .. } => {
                ...
                // consume the press — nothing in Unluminous does — so without this guard one press of
...
///
/// **Drawn over the line rather than put into the text**, which is the one place Unluminous's Markdown
...
/// selection it would make and changes nothing, which is the rule every component in Unluminous follows.
///
...
/// What the eleven box-drawing characters Unluminous writes are made of.
///
...
mod tests {
    use super::*;
    use crate::services::text_renderer::TextRenderer;
    use unluminous_core::{layout, CharStyle, ParagraphStyles, Rope, StyleSpans};
