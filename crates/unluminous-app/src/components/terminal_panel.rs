...
//!
//! The terminal itself is in `unluminous-terminal`, which has no user interface dependencies. This draws what
...
//! background colour, then the selection over them, then one glyph a cell out of Unluminous's own atlas, then the
...
use unluminous_terminal::keys::{self, KeyPress, Modifiers as TermModifiers};
use unluminous_terminal::mouse;
use unluminous_terminal::screen::{CursorShape, Screen};
use unluminous_terminal::session::{SelectionKind, SessionSettings, Size};
...
/// [`unluminous_terminal::Tabs::move_tab`], which is what `unluminous-cli terminal move` calls too, so a
...
pub(crate) fn grid(
    ui: &mut egui::Ui,
    area: Rect,
    session: Option<&mut unluminous_terminal::Session>,
    selecting: &mut bool,
    focused: bool,
    id: &str,
    empty: &str,
...
fn handle_input(
    ui: &mut egui::Ui,
    session: &mut unluminous_terminal::Session,
    selecting: &mut bool,
    focused: bool,
    ...
    // the ones something else in Unluminous also listens for: Escape closes every modal, Enter confirms one and opens
...
fn key_press(key: egui::Key, modifiers: &egui::Modifiers) -> Option<KeyPress> {
    use unluminous_terminal::keys::Key as TermKey;
    let control = modifiers.ctrl && !modifiers.mac_cmd;
...
/// `Ctrl+Space` is how a null is sent, and in Unluminous's terminal all three did nothing.
///
...
mod tests {
    ...
    fn control_and_a_letter_becomes_a_terminal_key_press() {
        ...
        assert_eq!(press.key, unluminous_terminal::keys::Key::Character('c'));
    ...
    fn the_special_keys_are_all_carried_across() {
        let plain = egui::Modifiers::default();
        for (key, expected) in [
            (egui::Key::Enter, unluminous_terminal::keys::Key::Enter),
            (egui::Key::Tab, unluminous_terminal::keys::Key::Tab),
            (egui::Key::Escape, unluminous_terminal::keys::Key::Escape),
            (egui::Key::ArrowUp, unluminous_terminal::keys::Key::Up),
            (egui::Key::Delete, unluminous_terminal::keys::Key::Delete),
            (egui::Key::F5, unluminous_terminal::keys::Key::Function(5)),
        ] {
