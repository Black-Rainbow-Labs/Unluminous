...
//! none of them runs a git command: each returns what was asked for and `UnluminousApp::run_action` sends
...
pub struct DialogOutcome {
    ...
    pub merge: Option<(String, unluminous_git::MergeOptions)>,
...
pub fn show(
    ...
    let (_, closed) = modal::show(ctx, "unluminous-git-dialog", width, height, |ui, area| {
