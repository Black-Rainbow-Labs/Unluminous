...
//! settled by `UnluminousApp::show_the_debug_tile` and its two siblings, not here.
//!
...
//!
//! Like every component in Unluminous this one takes a rectangle, draws itself and reports what happened.
...
pub struct Missing {
    ...
    pub sentence: String,
    /// The command that would install it, empty when Unluminous has nothing to offer.
    pub install: String,
}
...
pub struct DebugOutcome {
    ...
    pub step: Option<unluminous_dap::Step>,
...
fn show_header(
    ...
    // The adapter's own exception filters, when it offered any. **Unluminous holds no list of its own**,
...
fn show_watch(ui: &mut egui::Ui, row: Rect, watch: &Watch, outcome: &mut DebugOutcome) {
    ...
    // debugger explains a bad expression better than Unluminous could.
