...
//!
//! Unluminous had no menu on the editing area at all before `task-1663`. This is it: the ordinary
...
pub fn show(
    ...
    let popup = egui::Popup::new(
        egui::Id::new("unluminous-text-menu"),
        ui.ctx().clone(),
        menu.at,
        ui.layer_id(),
    )
    ...
    if let Some(response) = popup.show(|ui| {
        ...
        if let Some(colour) = menu.wheel {
            let (area, _) = ui.allocate_exact_size(
                Vec2::new(ui.available_width(), color_wheel::HEIGHT),
                Sense::hover(),
            );
...
fn blocks(
    ...
    for colour in HighlightColor::ALL {
        let area =
            Rect::from_min_size(Pos2::new(left, row.center().y - BLOCK / 2.0), Vec2::splat(BLOCK));
...
fn block(ui: &mut egui::Ui, area: Rect, colour: HighlightColor, enabled: bool) -> bool {
    ...
    let edge =
        if response.hovered() && enabled { color::text_strong() } else { color::control_border() };
    painter.rect_stroke(
        area,
        CornerRadius::same(6),
        Stroke::new(1.0, edge),
