...
//! `crates/unluminous-app/plugins/` holds agent-tasks, css, html, javascript, mermaid, rust and typescript, and
//! markdown is built into `unluminous-core` — `unluminous_core::markdown::render` is what the editor's own preview is made
...
//!
//! `unluminous_core::markdown::render` turns the source into the same three things a document holds — a rope, its
//! character styles and its paragraph styles — so `unluminous_core::layout` lays it out and
//! `components::editor_view::paint_text` paints it. This module is the join between them, and it exists because
//! `UnluminousApp::refresh_preview` is the only other place that made that join and it needs a tab: it reads the
...
//!
//! **Pictures and Mermaid diagrams.** `UnluminousApp::refresh_preview` resolves those in two further passes that
...
use egui::{Pos2, Rect};
use unluminous_core::{layout, Layout, Rope};
...
/// covers.
///
/// **`unluminous_core::markdown` already answers both** — `Preview::panels` and `Preview::code_spans` —
...
impl Rendered {
    ...
    ///
    /// Half a point of tolerance on the width, which is what `UnluminousApp::refresh_preview` uses: a pane whose
...
pub fn render(
    ...
    let advance = unluminous_core::FontMetrics::advance(renderer, "M", &code).max(1.0);
    let preview = unluminous_core::markdown::render(
        source,
        &unluminous_core::PreviewOptions {
            base: base.clone(),
            colors: unluminous_core::PreviewColors {
...
/// `Colors` value is: a component in Unluminous is handed the palette rather than reaching for one.
pub fn show_with(
    ui: &mut egui::Ui,
    area: Rect,
    rendered: &Rendered,
...
/// otherwise, which is exactly what `UnluminousApp::refresh_preview` does for a file. `tasks/agent-tasks-ui-tdd.md` §6
...
mod tests_task_28 {
    ...
    ///
    /// Rendering is `unluminous_core::markdown`'s, which has its own tests for what it produces. What this asserts is
