...
//!
//! What the reference capture in `tasks/unluminous-ide-tdd.md` holds, and what this draws:
//!
//! - a `Commit` / `Stashes` tab strip. The reference editor calls the second one `Shelf`; Unluminous has stashes, so
...
//! staging everything at the moment of commit — means Unluminous's idea of what is staged and git's
//! disagree for as long as the panel is open, and anyone who runs `git status` in Unluminous's own
...
pub fn show(
    ...
    let (_, closed) = modal::show(ctx, "unluminous-commit", WIDTH, HEIGHT, |ui, area| {
...
fn commit_tab(
    ...
    let summary =
        format!("{} added   {} modified", status.untracked_count(), status.modified_count());
    let galley =
        ui.painter().layout_no_wrap(summary, egui::FontId::proportional(11.5), color::git_added());
    ui.painter().galley(
        Pos2::new(
            counts.right() - galley.size().x,
            counts.center().y - galley.size().y / 2.0 + 4.0,
        ),
        galley,
        color::git_added(),
    );
...
fn recent_messages(ui: &mut egui::Ui, counts: Rect, panel: &mut CommitPanel, recent: &[String]) {
    ...
    let chosen = egui::Popup::new(
        egui::Id::new("unluminous-commit-messages"),
        ui.ctx().clone(),
        clock.left_bottom(),
        ui.layer_id(),
    )
...
fn changes_tree(
    ...
    egui::ScrollArea::vertical().id_salt("commit-changes").show(&mut list, |ui| {
        ...
        if status.is_clean() {
            ui.add_space(8.0);
            ui.label(
                egui::RichText::new("  Nothing has changed.").size(11.5).color(color::text_faint()),
            );
        }
    });
}
...
fn group_row(
    ...
    let response = modal::row(ui, id, heading, false, |painter, row| {
        ...
        let mut x =
            modal::label(painter, row, row.left() + 30.0, heading, color::text_strong(), 12.0);
...
fn file_row(
    ...
    let box_rect = Rect::from_center_size(
        Pos2::new(
            response.rect.left() + 14.0 + depth as f32 * size::INDENT,
...
fn stashes_tab(
    ...
    egui::ScrollArea::vertical().id_salt("stash-list").show(&mut child, |ui| {
        ...
        if stashes.is_empty() {
            ui.add_space(8.0);
            ui.label(
                egui::RichText::new("  Nothing is stashed.").size(11.5).color(color::text_faint()),
            );
        }
    });
