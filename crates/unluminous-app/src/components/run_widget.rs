...
//!
//! Three parts, and each is absent when it cannot apply, which is Unluminous's rule:
//!
...
//!   button would start **resolves to a debugger at all**, which `UnluminousApp::run_widget_state` works
...
//!   Unluminous's rule for a control that cannot apply, asked of the thing the button acts on rather than
...
//! rail and the keyboard all go down the one path in `UnluminousApp::run_action`.
...
pub struct WidgetState {
    ...
    /// file's language — see [`crate::app::UnluminousApp::adapter_for`].
    pub debuggable: bool,
...
pub fn show(ui: &mut egui::Ui, area: Rect, state: &WidgetState) -> Option<Action> {
    ...
    // Stop, drawn only while it can apply — Unluminous's rule for a control that cannot.
    if state.running {
