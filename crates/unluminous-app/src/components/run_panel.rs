...
//! holds a button each and pressing one puts the other away, which `UnluminousApp` settles, not this
//! file.
//!
//! ## What a run is
//!
...
//! never written into by Unluminous**: The reference editor prints its epilogue into the console, and that line
...
//! second press, is killed through [`unluminous_terminal::Session::kill`]. Closing a tab and closing the
...
pub struct Run {
    ...
    /// one that ended without choosing a code, which is a program Unluminous killed or, on Unix, one a
    /// signal took.
    ///
...
impl Run {
    ...
    pub fn state(&self) -> State {
        match self.ended {
            ...
            // A program whose code could not be read — one Unluminous killed, or one a signal took on
...
impl State {
    ...
    /// `theme::color`, which is the whole list of colours Unluminous draws with.
...
pub struct RunPanel {
    ...
    /// `UnluminousApp::run_grid_size` explains and what loses a fast program's output.
    pub tile: Rect,
}
...
impl RunPanel {
    ...
    pub fn start(
        ...
        root: &std::path::Path,
        size: Size,
        waker: unluminous_terminal::session::Waker,
    ) -> Result<usize, String> {
        ...
        let settings = SessionSettings {
            ...
            // variables after it so one that names the same variable still wins. An Unluminous started from
...
///
/// `controls::icon_button` has no such state, because until now every icon button in Unluminous could
...
fn draw_tab(
    ...
    shut_response.widget_info(|| {
        ...
        egui::WidgetInfo::labeled(egui::WidgetType::Button, true, format!("Close run {name}"))
    });
...
mod tests {
    ...
    fn a_run_that_has_ended_keeps_its_tab_and_says_what_it_ended_with() {
        ...
        // exercised by `unluminous-terminal`'s own tests and by the walkthrough.
