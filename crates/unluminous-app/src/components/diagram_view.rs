...
//!
//! `unluminous_core::mermaid` works out where everything goes and hands back a `Scene`: rectangles,
...
//! diagram type can be added to `unluminous-core` without a line changing here.
//!
...
//! blown up, which is what `fit` means everywhere else in Unluminous — the wheel scrolls it, a pinch or
...
/// short solid ones. Worked out here rather than in `unluminous-core` because how long a mark should be
/// depends on how large the diagram is being drawn, which only this side knows.
fn paint_line(
    painter: &egui::Painter,
    points: &[Pos2],
    stroke: &unluminous_core::mermaid::scene::Stroke,
    dash: Dash,
    scale: f32,
) {
...
fn stroke_of(stroke: &Option<unluminous_core::mermaid::scene::Stroke>, scale: f32) -> Stroke {
    match stroke {
...
/// A diagram's colour as egui knows it. Always fully opaque, like every other glyph in Unluminous.
fn colour_of(colour: unluminous_core::Color) -> Color32 {
...
/// type Unluminous does not draw yet is Unluminous's limitation and says so. Either way the **source is shown
/// underneath**, so nothing a person wrote disappears behind an error.
pub fn show_problem(
    ui: &egui::Ui,
    area: Rect,
    problem: &unluminous_core::mermaid::Problem,
    source: &str,
) {
    let painter = ui.painter();
