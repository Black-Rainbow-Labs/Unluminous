...
///
/// The default of `appearance.ui.font.size`, so an Unluminous that names no size in its settings file is drawn
...
impl LineEndings {
    ...
    pub fn applied_to(self, found: unluminous_core::LineEnding) -> unluminous_core::LineEnding {
        match self {
            LineEndings::Keep => found,
            LineEndings::Lf => unluminous_core::LineEnding::Lf,
            LineEndings::Crlf => unluminous_core::LineEnding::Crlf,
        }
    }
}
...
/// the off switch, because `Debug -> Show Value` and `unluminous-cli debug hover` work either way, so a
...
pub enum Page {
    ...
    Terminal,
    /// The Model Context Protocol server, which is how an AI agent drives Unluminous.
    Mcp,
...
impl Page {
    /// Unluminous's own six. A plugin's page is not in it, because there is no compile time list of them;
    /// [`Page::all`] is the one that includes them.
    pub const ALL: [Page; 6] =
        [Page::Appearance, Page::Theme, Page::Editor, Page::Plugins, Page::Terminal, Page::Mcp];

    /// Unluminous's own six, then one per plugin that contributed a page, in the order the plugins are
    /// listed.
...
impl Settings {
    ...
    /// startup, may have been written by an older or newer Unluminous, and the rule this file already
    /// keeps for a value it does not recognise is to fall back rather than refuse to start.
    pub fn mcp_area_filter(&self) -> unluminous_cli::mcp::tools::Areas {
        unluminous_cli::mcp::tools::Areas::parse(&self.mcp_areas).unwrap_or_else(|_| {
    ...
    /// they name none and Unluminous should look for it itself.
    ///
    ...
    ///
    /// A name Unluminous has not got reads as "follow the theme" rather than as an error, which is the rule
...
pub fn clamp_port(port: f32) -> u16 {
    port.clamp(unluminous_cli::mcp::MIN_PORT as f32, u16::MAX as f32) as u16
}
...
pub struct Panes {
    ...
    ///
    /// It lives here, in the person's own settings, rather than in the project's `.unluminous`: the
    /// window's *geometry* belongs to the project, because Unluminous's windows are one per project and a
...
mod tests {
    ...
    fn settings_survive_being_written_and_read_back() {
        let settings = Settings {
            ...
            mcp_enabled: true,
            mcp_port: 9001,
            mcp_tools: unluminous_cli::mcp::Shape::Every,
    ...
    /// when non-empty came back at the next start: choosing a theme and then choosing Unluminous Dark again
    ...
    fn a_setting_put_back_to_its_default_is_taken_out_of_the_file() {
        ...
        // And now put every one of them back to what an Unluminous that has never been run has.
    ...
    fn the_mcp_server_is_off_until_somebody_turns_it_on() {
        ...
        assert_eq!(fresh.mcp_port, unluminous_cli::mcp::DEFAULT_PORT);
        assert_eq!(fresh.mcp_tools, unluminous_cli::mcp::Shape::Grouped);
    }
