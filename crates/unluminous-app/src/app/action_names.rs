...
//! is the one thing a menu or a shortcut can ask for, so giving each one a name makes `unluminous-cli
//! action run <name>` a way in to everything the menus hold — including whatever is added to them
//! tomorrow, because `unluminous-cli action list` is built by walking the real menus rather than by
...
impl Action {
    ...
    pub fn name(&self) -> String {
        match self {
            ...
            // than from this list. `unluminous-cli action list` therefore offers them the day the manifest is
            ...
            Action::CheckForUpdates => "check-for-updates".to_owned(),
            Action::Find => "find".to_owned(),
            Action::Replace => "replace".to_owned(),
            ...
            Action::ToggleLineComment => "toggle-line-comment".to_owned(),
            Action::ToggleBlockComment => "toggle-block-comment".to_owned(),
            Action::DuplicateLines => "duplicate-line".to_owned(),
            Action::MoveLines { down: true } => "move-line-down".to_owned(),
            Action::MoveLines { down: false } => "move-line-up".to_owned(),
            Action::JoinLines => "join-lines".to_owned(),
            Action::SortLines => "sort-lines".to_owned(),
            Action::TrimTrailingWhitespace => "trim-trailing-whitespace".to_owned(),
            Action::GoToLine => "go-to-line".to_owned(),
            Action::GoToMatchingBracket => "go-to-matching-bracket".to_owned(),
            Action::CommandPalette => "command-palette".to_owned(),
            Action::ReopenClosedTab => "reopen-closed-tab".to_owned(),
            ...
            Action::ToggleMaximisedPane => "toggle-maximised-pane".to_owned(),
    ...
    pub fn from_name(name: &str, path: Option<PathBuf>) -> Option<Action> {
        ...
        Some(match name {
            ...
            "toggle-line-comment" => Action::ToggleLineComment,
            "toggle-block-comment" => Action::ToggleBlockComment,
            "duplicate-line" => Action::DuplicateLines,
            "move-line-down" => Action::MoveLines { down: true },
            "move-line-up" => Action::MoveLines { down: false },
            "join-lines" => Action::JoinLines,
            "sort-lines" => Action::SortLines,
            "trim-trailing-whitespace" => Action::TrimTrailingWhitespace,
            "go-to-line" => Action::GoToLine,
            "go-to-matching-bracket" => Action::GoToMatchingBracket,
            "command-palette" => Action::CommandPalette,
            "reopen-closed-tab" => Action::ReopenClosedTab,
            ...
            "toggle-maximised-pane" => Action::ToggleMaximisedPane,
...
impl GitAction {
    ...
    /// Every entry, which is what `unluminous-cli git actions` lists.
