//! What the menus hold, and one list of everything a menu can ask Unluminous to do.
//!
...
//!
//! [`Action`] is what a menu produces. Nothing here does anything: `UnluminousApp::run_action` is the one
...
pub enum Action {
    /// Another Unluminous window, on its own project.
    NewWindow,
    ...
    FindInFiles,
    /// Ask the releases page whether a newer Unluminous exists. `task-1804` §6.
    CheckForUpdates,
    ...
    /// which is what every honest miss in Unluminous does.
    CompleteWord,
    ...
    /// hiding the last panel while it is hidden brings it back. See `UnluminousApp::run_action`.
    ToggleEditor,
    ...
    /// carry a colour and an opacity. Both go through `UnluminousApp::highlight_selection`, so there is
    ...
    Fold(FoldAction),
    /// Unluminous's own about box, which is a line in the status bar rather than a window.
    About,
    Quit,
...
/// `unluminous-cli action run` can all ask for. `tasks/task-1686-folding-tdd.md` section 8, and
...
impl HighlightColor {
    ...
    pub fn rgba(&self) -> unluminous_core::Rgba {
    ...
    pub fn parse(name: &str) -> Option<unluminous_core::Rgba> {
        let name = name.trim();
        ...
        unluminous_core::Rgba::parse(name)
    }
...
impl Shortcut {
    /// The command key and a key, which is nearly every shortcut Unluminous has.
...
pub struct MenuState {
    ...
    /// window already holds and its `Default` is the arrangement Unluminous ships with.
...
///
/// `Unluminous` comes first because that is where the application's own entries belong, and because macOS puts
...
/// `Ctrl+R` on macOS. They differ because the platforms do, not because Unluminous has an opinion.
...
/// differ because the platforms do, not because Unluminous has an opinion.
...
///
/// Every entry is an [`Action`] dispatched in `UnluminousApp::run_action`, so the widget, the menu, the
...
///
/// **Absent altogether when the file's language names no debugger**, which is Unluminous's rule for a
...
/// The Git menu, which holds what the reference capture in `tasks/unluminous-ide-tdd.md` holds.
///
...
fn unluminous_menu() -> Menu {
    Menu {
        name: "Unluminous".to_owned(),
        // Settings is in two menus on purpose. `tasks/improvements.md` names both `Unluminous -> Settings` and
        ...
        entries: vec![
            Entry::item("About Unluminous", Action::About),
            // **A person asking**, which is the only thing that makes Unluminous send anything on its
            ...
            Entry::Separator,
            Entry::with_shortcut("Quit Unluminous", Action::Quit, Shortcut::command(egui::Key::Q)),
        ],
    }
}
...
///
/// They are **absent** when the file's language cannot answer them, which is Unluminous's rule for a
...
pub fn symbol_entries(state: &MenuState) -> Vec<Entry> {
    ...
    if state.definitions_apply {
        // The reference editor's key. The command key and B is bold everywhere else in Unluminous, and the two can
...
/// editor keeps this under `Edit -> Find`, one level further down. Unluminous's Edit menu is eight
...
fn view_menu(state: &MenuState) -> Menu {
    ...
    Menu {
        ...
        entries: vec![
            ...
            // real menu at all so that `unluminous-cli action list` finds them, because that list is built
...
/// the View menu, the keyboard and `unluminous-cli action run` can all ask for without inventing a way to
...
/// still find them, and so `unluminous-cli action list` — which is built by walking the real menus — lists
/// them.
...
/// is what makes them parameterless actions the View menu, the keyboard and `unluminous-cli action run`
...
/// can still find them, and so `unluminous-cli action list` — which is built by walking the real menus —
/// lists them.
...
/// `unluminous-cli panel`, which is a whole area of the catalogue and is what an agent reads.
...
pub fn explorer_menu(
    ...
    // absent is Unluminous's rule for a control that can never apply, and every one of these is live the
...
/// puts it on the command line the day it exists — `unluminous-cli action list` is built by walking the
...
///
/// Absent altogether for a file that cannot fold, which is Unluminous's rule for a control that can never
...
pub fn gutter_menu(state: &MenuState) -> Vec<Entry> {
    ...
    // the text menu and the terminal tab menu already follow; `UnluminousApp::gutter_menu_line` is what
...
mod tests {
    ...
    #[test]
    fn unluminous_comes_first_and_then_file_edit_find_view_and_git() {
        ...
        assert_eq!(order, vec!["Unluminous", "File", "Edit", "Find", "View", "Run", "Git"]);
    }
    ...
    /// The debug half is **absent** for a language that names no debugger, which is Unluminous's rule for
    ...
    fn settings_can_be_reached_from_the_application_menu_and_from_the_edit_menu() {
        let bar = menus(&MenuState::default());
        assert!(names(&find(&bar, "Unluminous").entries).contains(&"Settings".to_owned()));
