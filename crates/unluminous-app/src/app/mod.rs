...
/// A file being carried out of the explorer or a Folder node — `task-1914`.
///
/// The same shape as [`TabDrag`] and for the same reason: the list a row was picked up in reports that it
/// is in the air and where the pointer is, and it cannot know where that turned out to be, because the
/// answer may be a node it has never heard of. [`UnluminousApp::settle_the_file_drag`] decides once every
/// panel and every node has been drawn.
///
/// `at` is in the **window's** own points, converted by whichever list reported it.
#[derive(Debug, Clone, PartialEq)]
struct FileDrag {
    path: PathBuf,
    at: Pos2,
...
impl UnluminousApp {
        ...
        if self.explorer_visible && explorer_rect.width() > 1.0 && explorer_rect.height() > 1.0 {
            ...
            if explorer_outcome.moved.is_none() {
                if let Some((path, at, dropped)) = explorer_outcome.carrying {
                    self.file_drag = Some(FileDrag { path, at, dropped });
                }
            }
    ...
    fn settle_the_tab_drag(&mut self, ui: &mut egui::Ui, pane_rects: &[Rect]) {
        ...
        if self.canvas_would_take_a_drop_at(drag.at) {
            if drag.dropped {
    ...
    fn settle_the_file_drag(&mut self, ui: &mut egui::Ui) {
        ...
        if drag.dropped {
            if let Err(problem) = self.drop_a_file_onto_the_canvas(&drag.path, drag.at) {
                self.message = Some(problem);
            }
            return;
        }
