...
//! away.
//!
//! `task-1696`. `unluminous_core::expressions` reads what the pointer is over, `app::debug::DebugState`
...
//! so somebody who finds the tooltip distracting can turn it down, and Unluminous's tick box already
...
/// only the *unasked* popup — `Debug -> Show Value` and `unluminous-cli debug hover` work either way,
...
impl UnluminousApp {
    ...
    ///
    /// One function, so the pointer, `Debug -> Show Value` and `unluminous-cli debug hover` cannot come to
    ...
    pub fn expression_at(&mut self, index: usize, offset: usize) -> Option<Range<usize>> {
        ...
        unluminous_core::expressions::at(&text, symbols, &grammar, offset)
    }
    ...
    fn word_box(&mut self, index: usize, range: &Range<usize>, origin: Pos2) -> Rect {
        ...
        let box_of = |caret: unluminous_core::layout::Caret, right: f32| {
            Rect::from_min_max(
