...
//!
//! Nothing here draws and nothing here speaks the protocol. The protocol is `unluminous_dap`, the drawing
//! is `components::debug_panel` and `components::gutter`, and this is what sits between them —
//! exactly the place `app::git::GitState` sits between `unluminous_git::Worker` and `components::git_panel`.
//!
//! **One session per window.** the reference editor runs several; the first version of this does not, and
//! everything is simpler for it: the state below is one `Option<DebugState>` on `UnluminousApp` rather
...
//! `unluminous_dap::Request::resumes` and acted on here. What is **not** thrown away is which rows were
...
//!
//! Unluminous draws the adapter's answer rather than its own hope. [`DebugState::answered`] holds, per
...
/// The build runs **on a thread**, the way `unluminous-git` runs git, because a cold `cargo build` of this
/// repository is minutes and the window draws throughout it. The reply arrives on a channel and is
/// taken in `UnluminousApp::take_the_debug_replies`, beside every other thread's replies.
///
...
pub struct PendingBuild {
    ...
    /// What the tile says while it runs — `Building unluminous`.
    pub what: String,
...
pub enum Built {
    ...
    /// rule `unluminous-git` already follows about git's standard error.
    Failed(String),
...
pub struct HoverValue {
    ...
    pub id: u64,
    /// The expression as `unluminous_core::expressions::at` read it, which is also the root row's key.
    pub expression: String,
...
impl DebugState {
    ...
    ///
    /// The other half of [`unluminous_dap::Client::detached`], and it exists for the same reason
    ...
    pub fn feed(&mut self, message: unluminous_dap::Message) {
    ...
    /// itself.
    pub fn capabilities(&self) -> &unluminous_dap::Capabilities {
    ...
    fn send(&mut self, outcome: Outcome) -> bool {
        ...
        // The events an operation produces are about what Unluminous has just decided rather than about
    ...
    /// the third place deliberately, for the reason `unluminous_terminal::paths` gives about the verbatim
    ...
    /// real one answers `'total' is not a valid command`. Unluminous's box is called `Evaluate Expression`
    /// and the reference editor's evaluates an expression, so `watch` is the context that means what the control
    /// says. The distinction Unluminous keeps between the two is the reference editor's — persistent against one-off —
    /// which is Unluminous's own bookkeeping rather than anything on the wire.
    ...
    ///    Unluminous's own registry prefers, the first way alone would mean the commonest thing anybody
    ...
    /// is Unluminous's rule that a control which can never apply is absent. The children of a tooltip are
...
mod tests {
    ...
    fn answer(request_seq: i64, command: &str, body: serde_json::Value) -> unluminous_dap::Message {
        unluminous_dap::Message::Response {
            seq: 500 + request_seq,
            request_seq,
    ...
    /// And anything that is not a name the frame holds has no field at all, which is Unluminous's rule
    ...
    fn resuming_puts_the_tooltip_away() {
        ...
        state.step(unluminous_dap::Step::Over).expect("stopped, so it can step");
