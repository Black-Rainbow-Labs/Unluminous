...
//!
//! `unluminous_core::folding` reads a file for what **could** be collapsed and holds which of them are;
...
use unluminous_core::folding::{self, Folds, Hidden, Region};
...
impl UnluminousApp {
    ...
    ///
    /// `unluminous-cli fold others --selection` is the only thing that says so: an agent that has just
    ...
    /// something other than a click — a jump to a definition, a search hit, `unluminous-cli editor caret
...
mod tests {
    ...
    fn collapsed(app: &mut UnluminousApp) -> Vec<usize> {
    ...
    fn expanding_recursively_opens_the_children_that_were_folded_on_their_own() {
        let (folder, mut app) = a_window("unluminous-fold-recursive-expand");
        let index = app.files.active_index();
    ...
    fn collapsing_recursively_twice_is_a_no_op_the_second_time() {
        let (folder, mut app) = a_window("unluminous-fold-recursive-noop");
    ...
    fn a_caret_inside_a_recursively_collapsed_block_is_brought_onto_its_head() {
        let (folder, mut app) = a_window("unluminous-fold-recursive-caret");
        let index = app.files.active_index();
