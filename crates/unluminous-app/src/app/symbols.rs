...
//!
//! The index is a picture of the disk as it was when it was last read. A file changed outside Unluminous
...
//! confirmed to still be there, re-found by [`unluminous_core::symbols::file_definitions`] if it has
//! moved. `UnluminousApp::open_the_match` already re-checks a search hit the same way. A stale entry
...
impl UnluminousApp {
    ...
    /// started lazily rather than in `UnluminousApp::new`, because a window that never asks a question
    ...
    /// `unluminous_core::symbols::rank`, which is where it can be tested with no window.
    fn rank_candidates(
        &self,
    ...
    /// Apply Unluminous's honest navigation rule to a ranked set of definition candidates.
    ...
    fn jump_to(&mut self, candidate: &Candidate, name: &str) {
        ...
        let Some(range) = range else {
            self.message = Some(format!(
                "'{name}' is no longer in {}, so Unluminous could not go to it.",
...
mod tests {
    ...
    /// by counting bytes.
    fn at(app: &UnluminousApp, needle: &str) -> usize {
    ...
    fn a_definition_in_another_file_opens_that_file_with_the_name_selected() {
        // Scenario 2.
        let (folder, mut app) = a_window("unluminous-symbols-another-file");
    ...
    fn a_word_with_no_definition_says_so_rather_than_guessing() {
        ...
        let (folder, mut app) = a_window("unluminous-symbols-no-definition");
        app.open_path_permanently(&folder.join("caret.rs")).expect("the file opens");
    ...
    fn asking_from_the_definition_itself_pivots_to_the_references() {
        ...
        let (folder, mut app) = a_window("unluminous-symbols-pivot");
    ...
    fn two_files_defining_one_name_offer_both_rather_than_choosing() {
        ...
        let (folder, mut app) = a_window("unluminous-symbols-two-candidates");
        app.open_path_permanently(&folder.join("caret.rs")).expect("the file opens");
        ...
        app.document_mut()
            .apply(unluminous_core::Command::PlaceCaret { offset: use_site, extend: false });
        app.document_mut().apply(unluminous_core::Command::Insert("new();\n        ".to_owned()));
    ...
    fn a_file_deleted_since_it_was_indexed_reports_and_does_not_crash() {
        // Scenario 12.
        let (folder, mut app) = a_window("unluminous-symbols-deleted");
        app.open_path_permanently(&folder.join("caret.rs")).expect("the file opens");
    ...
    fn going_back_through_two_jumps_and_forward_again() {
        // Scenario 18.
        let (folder, mut app) = a_window("unluminous-symbols-navigate");
        app.open_path_permanently(&folder.join("caret.rs")).expect("the file opens");
        let started = app.document().selection().head;
        let offset = at(&app, "layout.draw()") + "layout.".len();
        app.document_mut().apply(unluminous_core::Command::PlaceCaret { offset, extend: false });
    ...
    fn asking_with_the_caret_on_nothing_says_so_and_opens_no_modal() {
        // Scenario 26.
        let (folder, mut app) = a_window("unluminous-symbols-whitespace");
        app.open_path_permanently(&folder.join("layout.rs")).expect("the file opens");
        let blank = at(&app, "\n\nimpl");
    ...
    fn the_symbol_entries_are_absent_for_a_file_whose_language_cannot_answer() {
        ...
        let (folder, mut app) = a_window("unluminous-symbols-absent");
    ...
    fn a_rename_edits_the_open_tab_as_a_document_and_the_closed_files_on_the_disk() {
        ...
        let (folder, mut app) = a_window("unluminous-symbols-rename-split");
        app.open_path_permanently(&folder.join("layout.rs")).expect("the file opens");
        ...
        app.document_mut().apply(unluminous_core::Command::Undo);
    ...
    fn a_closed_file_that_changed_since_the_search_is_skipped_whole_and_reported() {
        ...
        let (folder, mut app) = a_window("unluminous-symbols-changed-underneath");
    ...
    fn a_closed_files_stored_marks_are_shifted_by_the_rename_that_moved_its_bytes() {
        ...
        let (folder, mut app) = a_window("unluminous-symbols-rename-marks");
    ...
    fn a_comment_or_a_string_ticked_by_hand_is_applied_like_any_other_row() {
        ...
        let (folder, mut app) = a_window("unluminous-symbols-rename-a-comment");
        let caret = folder.join("caret.rs");
        let text = std::fs::read_to_string(&caret).expect("read");
    ...
    fn a_large_project_costs_definitions_rather_than_occurrences_and_still_cancels_at_once() {
        ...
        let folder = std::env::temp_dir().join("unluminous-symbols-large");
        std::fs::remove_dir_all(&folder).ok();
        std::fs::create_dir_all(&folder).expect("make the folder");
    ...
    fn a_rename_with_no_files_in_it_changes_nothing_and_says_so() {
        ...
        let (folder, mut app) = a_window("unluminous-symbols-rename-nothing");
        let report = app.apply_rename(&RenameChange {
            from: "draw".to_owned(),
            to: "sketch".to_owned(),
            by_file: Vec::new(),
    ...
    fn two_panes_showing_two_affected_files_both_take_the_edit_once() {
        ...
        let (folder, mut app) = a_window("unluminous-symbols-two-panes");
        app.open_path_permanently(&folder.join("layout.rs")).expect("the file opens");
    ...
    fn what_a_build_wrote_is_never_indexed_or_searched() {
        ...
        let folder = a_project("unluminous-symbols-generated");
    ...
    fn a_reference_search_reads_an_open_tab_as_it_stands_and_the_disk_for_the_rest() {
        ...
        let (folder, mut app) = a_window("unluminous-symbols-open-texts");
        app.open_path_permanently(&folder.join("layout.rs")).expect("the file opens");
    ...
    fn the_command_line_can_open_a_definition_by_name_without_finding_an_occurrence_first() {
        let (folder, mut app) = a_window("unluminous-symbols-cli-name");
        let context = egui::Context::default();
    ...
    fn the_command_line_refuses_a_new_name_this_language_could_not_hold() {
        ...
        let (folder, mut app) = a_window("unluminous-symbols-cli-refusal");
        let context = egui::Context::default();
        app.open_path_permanently(&folder.join("layout.rs")).expect("the file opens");
