...
//!
//! **Nothing is ever fetched.** Zed downloads its adapters; Unluminous does not, because the rule that
...
//! sentence, as the debug tile's Install button, and as `unluminous-cli debug adapters` and
...
//! `%LOCALAPPDATA%\Unluminous\adapters` — no elevation, fifty megabytes rather than LLVM's two and a half
...
//! context answers `'total' is not a valid command`. So Unluminous's `Evaluate Expression` asks in the
...
//! the protocol channel about why. It is CodeLLDB's fault rather than Unluminous's, and what Unluminous does
...
//!
//! **What about Python?** Unluminous ships no Python plugin, so there is no `python` entry — an entry no
...
use unluminous_dap::AdapterCommand;
...
/// One debugger Unluminous knows how to drive.
pub struct Debugger {
    ...
    /// refusal into something a person or an agent can act on without leaving Unluminous.
    pub install: Install,
...
impl Install {
    /// The command for the machine Unluminous is running on.
...
impl Debugger {
    ...
    /// answer means Unluminous has nothing to offer here, and the refusal then says what the settings key
...
/// installed Unluminous that has no source checkout beside it — and because a person reading it in the
/// status bar can see exactly what it will do before pressing anything. **Unluminous does not run this by
...
pub const CODELLDB_FETCH: &str = concat!(
    ...
    r"$into=Join-Path $env:LOCALAPPDATA 'Unluminous\adapters'; ",
...
pub const JS_DEBUG_FETCH_UNIX: &str = concat!(
    "sh -c \"",
    "set -e; into=$HOME/.local/share/unluminous/adapters; mkdir -p $into; ",
...
/// Every debugger built into this version of Unluminous.
///
...
pub const ALL: &[Debugger] = &[
    Debugger {
        ...
        // On Windows, CodeLLDB's own `.vsix` unpacked into Unluminous's adapters folder: it is a zip
        ...
        // written out as one command so an installed Unluminous with no source checkout can offer it.
    ...
    Debugger {
        ...
        // installing the extension is not a way to get the thing Unluminous needs, and the release asset
        // is.
        install: Install {
...
pub fn prepare(
    ...
    let entry = find(name).ok_or_else(|| Refusal::NotInstalled {
        name: name.to_owned(),
        looked_for: Vec::new(),
        comes_from: "this version of Unluminous does not know it".to_owned(),
        install: String::new(),
    })?;
    ...
    match name {
        ...
        _ => Err(Refusal::NotInstalled {
            name: name.to_owned(),
            looked_for: Vec::new(),
            comes_from: "this version of Unluminous does not know how to start it".to_owned(),
            install: String::new(),
        }),
    }
}
...
fn prepare_lldb(
    ...
    if let Some(tool) = build_tool(&program) {
        return Err(Refusal::BuildTool {
            ...
            advice: "Unluminous debugs `cargo run` and `cargo test` by building them first; anything else needs a configuration that names the built binary, such as `target\\debug\\myapp.exe`.".to_owned(),
        });
    }
    ...
    Ok(Launch {
        ...
        body: json!({
            ...
            // The one place Unluminous asks for the debuggee to be run in its own terminal, which is
...
/// as it was written; `PATH` is the machine's own answer; [`well_known`] is Unluminous guessing, correctly
...
fn codelldb() -> Option<PathBuf> {
    ...
    // Unluminous's own folder first, which is where `tools/get-debug-adapter.ps1` and the Install button
    ...
    if let Some(own) = unluminous_adapters()
...
/// Where Unluminous keeps an adapter it unpacked for itself.
///
...
fn js_debug() -> Option<PathBuf> {
    // Unluminous's own folder first, which is where the release asset unpacks to.
    if let Some(own) = unluminous_adapters()
...
/// Where `program` is on the `PATH` Unluminous searches, if it is there.
///
...
/// simply this process's own: an Unluminous started from the Finder has four folders on it and none of them
...
/// `UnluminousApp::adapter_for`, for a command line none of this recognises.
///
...
/// What `unluminous-cli debug adapters` says about one debugger, and what the debug tile's own panel
...
pub struct Report {
    ...
    pub comes_from: &'static str,
    /// The command that installs it on this machine, empty when Unluminous has nothing to offer.
    pub install: String,
...
mod tests {
    ...
    fn cargo_is_debugged_as_the_binary_it_built_rather_than_being_refused() {
        ...
        assert_eq!(prepared.body["program"], "C:\\p\\target\\debug\\unluminous.exe");
    ...
    fn a_name_that_is_a_path_is_not_looked_for_on_the_path() {
        ...
        assert!(on_path("unluminous-no-such-program-anywhere").is_none());
    }
