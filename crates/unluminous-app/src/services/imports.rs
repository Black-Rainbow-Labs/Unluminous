...
//!
//! `unluminous_core::imports` reads the text and says what the caret is in the middle of; this is the
...
//! over paths, and walking `unluminous_core::completion` down a folder tree is the same arithmetic with
...
use unluminous_core::syntax::{Grammar, PathRoot};
...
/// package, and one with a scheme in it would be a fetch, which nothing in Unluminous ever does.
...
pub struct Module {
    ...
    pub root: PathBuf,
    /// How the package is spelt in a path: `unluminous_app`.
    pub package: String,
...
mod tests {
    use super::*;
    use unluminous_core::syntax::ImportStyle;
    ...
    /// A little Rust workspace, shaped like Unluminous's own.
    fn workspace() -> (PathBuf, Vec<PathBuf>) {
        let root = PathBuf::from("/unluminous");
        let files = [
            "crates/unluminous-core/src/lib.rs",
            "crates/unluminous-core/src/completion.rs",
            "crates/unluminous-core/src/mermaid/mod.rs",
            "crates/unluminous-core/src/mermaid/layered.rs",
            "crates/unluminous-app/src/lib.rs",
            "crates/unluminous-app/src/app/mod.rs",
            "crates/unluminous-app/src/app/actions.rs",
            "crates/unluminous-app/src/components/editor_view.rs",
        ]
        .iter()
        .map(|name| root.join(name))
        .collect();
    ...
    fn a_module_path_walks_the_source_root_of_the_package_it_names() {
        ...
        let project = project(&root, &files);
        let from = root.join("crates/unluminous-app/src/components/editor_view.rs");
        let reach = |path: &[&str]| {
        ...
        assert_eq!(
            ...
            Some(Reached {
                folder: None,
                file: Some(root.join("crates/unluminous-app/src/app/actions.rs")),
            })
        );
        assert_eq!(
            reach(&["unluminous_core", "completion"]),
            Some(Reached {
                folder: None,
                file: Some(root.join("crates/unluminous-core/src/completion.rs")),
            })
        );
        ...
        assert_eq!(reach(&["unluminous-core", "completion"]), None);
    }
    ...
    fn a_folder_with_a_module_file_answers_with_both() {
        let (root, files) = workspace();
        let project = project(&root, &files);
        let from = root.join("crates/unluminous-core/src/lib.rs");
        let segments = ["crate".to_owned(), "mermaid".to_owned()];
        assert_eq!(
            resolve_segments(&project, &from, &segments, &rust()),
            Some(Reached {
                folder: Some(root.join("crates/unluminous-core/src/mermaid")),
                file: Some(root.join("crates/unluminous-core/src/mermaid/mod.rs")),
            })
        );
    }
    ...
    fn an_unresolved_segment_answers_nothing_rather_than_everything() {
        ...
        let project = project(&root, &files);
        let from = root.join("crates/unluminous-app/src/app/mod.rs");
        let typo = ["unluminou_core".to_owned(), "completion".to_owned()];
    ...
    fn nothing_written_offers_the_reserved_roots_and_the_packages() {
        ...
        assert_eq!(
            ...
            vec![
                ...
                "self".to_owned(),
                "super".to_owned(),
                "unluminous_core".to_owned(),
                "unluminous_app".to_owned(),
            ]
        );
    }
