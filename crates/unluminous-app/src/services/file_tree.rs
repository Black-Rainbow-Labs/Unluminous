...
//! expanded rather than at startup, so opening Unluminous on a large folder is not slow.
//!
//! Every file is listed, not only the ones Unluminous can open. An earlier version listed `.md` and `.txt` only,
...
//! are shown, and the ones Unluminous cannot open are drawn dimmed and do not respond to a click, which says
...
//! file holding text opens, whether or not Unluminous has any special handling for that kind of text.
...
pub struct Entry {
    ...
    /// True when Unluminous can open this file. A file it cannot open is still listed, but dimmed.
    pub openable: bool,
...
/// How deep the filter and the file count look, so that pointing Unluminous at a very large folder cannot make
...
impl FileTree {
    ...
    /// How many files there are under the root, whether or not Unluminous can open them.
    pub fn file_count(&self) -> usize {
        self.all_files.len()
    }

    /// How many of those files Unluminous can open.
    ///
...
fn read_directory(path: &Path) -> std::io::Result<Vec<Entry>> {
    ...
    for entry in std::fs::read_dir(path)? {
        ...
        // the honest answer: something is there and Unluminous cannot say what.
...
mod tests_task_28 {
    ...
    /// `task-28`: expanding a folder froze Unluminous and it had to be force quit.
    ///
    ...
    fn a_folder_holding_a_pipe_is_read_without_opening_it() {
        ...
        assert!(!entry.entry.openable, "a pipe is not something Unluminous opens");
        assert_eq!(
            entry.entry.refusal,
...
mod tests_task_1693 {
    ...
    fn a_file_made_by_something_else_shows_as_a_change() {
        let root = std::env::temp_dir().join("unluminous-tree-watch-new-file");
        let _ = std::fs::remove_dir_all(&root);
        std::fs::create_dir_all(&root).expect("make the folder");
...
mod tests {
    ...
    fn a_new_tree_shows_folders_first_then_every_file() {
        let root = sample_folder("unluminous-tree-order");
        let tree = FileTree::new(&root);
        let names: Vec<&str> = tree.rows().iter().map(|row| row.entry.name.as_str()).collect();
        assert_eq!(
            names,
    ...
    fn what_a_build_wrote_is_shown_in_the_tree_and_left_out_of_what_is_searched() {
        let root = sample_folder("unluminous-tree-build-output");
    ...
    #[test]
    fn a_file_unluminous_cannot_open_is_listed_but_marked_as_not_openable() {
        let root = sample_folder("unluminous-tree-openable");
        let tree = FileTree::new(&root);
        let entry = |name: &str| {
            tree.rows()
                .iter()
    ...
    fn a_folder_is_closed_until_it_is_opened() {
        let root = sample_folder("unluminous-tree-closed");
        let mut tree = FileTree::new(&root);
    ...
    fn opening_a_folder_indents_its_children_one_step() {
        let root = sample_folder("unluminous-tree-indent");
        let mut tree = FileTree::new(&root);
        tree.toggle(&root.join("notes"));
    ...
    fn closing_a_folder_hides_its_children_again() {
        let root = sample_folder("unluminous-tree-close");
        let mut tree = FileTree::new(&root);
        tree.toggle(&root.join("notes"));
    ...
    fn expanding_a_deep_path_opens_every_folder_above_it() {
        let root = sample_folder("unluminous-tree-expand");
        let mut tree = FileTree::new(&root);
    ...
    fn children_are_read_only_when_a_folder_is_opened() {
        let root = sample_folder("unluminous-tree-lazy");
        let mut tree = FileTree::new(&root);
    ...
    fn reloading_keeps_the_folders_that_were_open() {
        let root = sample_folder("unluminous-tree-reload");
        let mut tree = FileTree::new(&root);
        tree.toggle(&root.join("notes"));
    ...
    fn the_file_count_includes_files_in_folders_that_have_not_been_opened() {
        let root = sample_folder("unluminous-tree-count");
        let tree = FileTree::new(&root);
    ...
    fn the_filter_finds_a_file_inside_a_folder_that_was_never_opened() {
        let root = sample_folder("unluminous-tree-filter");
        let tree = FileTree::new(&root);
        assert!(
    ...
    fn the_filter_ignores_case_and_matches_part_of_a_name() {
        let root = sample_folder("unluminous-tree-filter-case");
        let tree = FileTree::new(&root);
    ...
    fn a_filter_result_reports_how_deep_it_sits() {
        let root = sample_folder("unluminous-tree-depth");
        let tree = FileTree::new(&root);
    ...
    fn a_folder_that_cannot_be_read_reports_why_rather_than_looking_empty() {
        let missing = std::env::temp_dir().join("unluminous-tree-does-not-exist-at-all");
