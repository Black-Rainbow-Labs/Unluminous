...
fn contributions(values: &Values, kind: Kind) -> Result<Contributions, String> {
    let provider = match word(values, "ui.provider") {
        ...
        Some(named) => {
            return Err(format!(
                "ui.provider is `{named}`, and this version of Unluminous has {}",
                UI_PROVIDERS.join(", ")
            ))
        }
    ...
    let chrome = match word(values, "ui.chrome") {
        ...
        Some(named) => {
            return Err(format!(
                "ui.chrome is `{named}`, and this version of Unluminous has {}",
                CHROME.join(", ")
            ))
        }
        None => None,
...
fn pane(values: &Values) -> Result<Option<PaneContribution>, String> {
    ...
    let icon = match word(values, "pane.icon") {
        ...
        Some(named) => {
            return Err(format!(
                "pane.icon is `{named}`, and this version of Unluminous draws {}",
                PANE_ICONS.join(", ")
            ))
        }
        None => "board".to_owned(),
    ...
    let applies = match word(values, "pane.applies") {
        ...
        Some(named) => {
            return Err(format!(
                "pane.applies is `{named}`, and this version of Unluminous knows {}",
...
fn page(values: &Values) -> Result<Option<PageContribution>, String> {
    ...
    let icon = match word(values, "settings.icon") {
        ...
        Some(named) => {
            return Err(format!(
                "settings.icon is `{named}`, and this version of Unluminous draws {}",
                PANE_ICONS.join(", ")
            ))
        }
        None => "board".to_owned(),
...
fn run_project(values: &Values) -> Result<Option<String>, String> {
    ...
    Err(format!(
        "run.project is `{named}`, and this version of Unluminous detects {}",
...
fn debug_adapter(values: &Values) -> Result<Option<String>, String> {
    ...
    Err(format!(
        "debug.adapter is `{named}`, and this version of Unluminous drives {}",
        DEBUGGERS.join(", ")
    ))
}
...
fn definers(values: &Values) -> Result<Vec<(String, SymbolKind)>, String> {
    ...
    for entry in list(values, "language.definers") {
        ...
        let Some(parsed) = SymbolKind::parse(kind) else {
            ...
            return Err(format!(
                "language.definers says `{entry}`, and a definition in Unluminous is one of {}",
                known.join(", ")
            ));
        };
...
fn path_roots(values: &Values) -> Result<Vec<(String, PathRoot)>, String> {
    ...
    for entry in list(values, "language.path_roots") {
        ...
        let Some(parsed) = PathRoot::parse(meaning) else {
            ...
            return Err(format!(
                "language.path_roots says `{entry}`, and a root in Unluminous is one of {}",
                known.join(", ")
            ));
        };
        let word = word.trim();
