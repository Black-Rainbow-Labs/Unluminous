...
///
/// The flag itself stays, because it is not a check in Unluminous: it is what asks a **server** for a
...
fn one(values: &Values, index: usize) -> Result<Source, String> {
    ...
    let engine = Engine::from_name(engine_name).ok_or_else(|| {
        format!(
            "`{name}` names the engine `{engine_name}`, and this version of Unluminous speaks {}.",
            Engine::ALL.join(", ")
        )
    })?;
    ...
    match engine {
        ...
        Engine::Postgres => {
            ...
            source.ssl = SslMode::from_name(mode).ok_or_else(|| {
                format!(
                    "`{}` names sslmode `{mode}`, and Unluminous has {}.",
                    source.name,
                    SslMode::ALL.join(", ")
                )
            })?;
    ...
    if values.text(&at("password")).is_some() {
        return Err(format!(
            "`{}` has a `password` in the file. Unluminous does not store one: use `password.env` to \
...
mod tests {
    ...
    fn a_configuration_round_trips_and_carries_no_secret() {
        ...
        assert!(text.contains("UNLUMINOUS_DB_AI"), "the name of the variable is written down");
