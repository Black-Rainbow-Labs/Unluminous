...
//! `crates/unluminous-db/tests/scripted_server.rs` is where the wire protocol is tested, and
//! `cargo run -p unluminous-db --example connect` is how the real server is.
...
fn a_console_runs_the_statement_under_the_caret() {
    // Which is the whole reason `unluminous_db::sql::at` exists: a console holds several statements and
...
fn the_view_answers_the_same_numbers_the_pane_is_drawing() {
    // Unluminous's rule: a pane drawn with `egui` is invisible to a test and to an agent unless it can be
...
fn where_a_password_is_can_be_said_on_the_command_line_and_the_password_itself_cannot() {
    ...
    run(&mut explorer, "password", &["test", "env", "UNLUMINOUS_DB_TEST"]);
    assert_eq!(
    ...
    assert_eq!(
        ...
        Some(Secret::Environment("UNLUMINOUS_DB_REMOTE".to_owned()))
    );
    explorer
    ...
    // There is deliberately no way to give Unluminous the password itself: it would be in a shell history,
...
fn a_grids_statement_asks_for_one_more_row_than_it_keeps() {
    ...
    let grid = Grid {
        ...
        at: 2,
        rows: unluminous_db::Rows::default(),
        pending: unluminous_db::Pending::default(),
        failure: None,
        running: None,
        chosen: None,
        editing: None,
    };
    let statement = select_for(&grid, unluminous_db::Engine::Sqlite, 200, 2);
    assert_eq!(
        statement,
    ...
    assert_eq!(
        select_for(&unordered, unluminous_db::Engine::Postgres, 200, 0),
    ...
    assert_eq!(
        select_for(&compound, unluminous_db::Engine::Postgres, 10, 0),
    ...
    let no_key = Grid {
        table: unluminous_db::Table { key: Vec::new(), ..unordered.table.clone() },
        ..unordered.clone()
    };
    assert_eq!(
        select_for(&no_key, unluminous_db::Engine::Postgres, 10, 0),
        "select * from \"member\" limit 11 offset 0"
    );
    // A SQLite table addressed by its `rowid` has to ask for it by name: it is not in `select *`.
    let by_rowid = Grid {
        table: unluminous_db::Table { key: vec!["rowid".to_owned()], ..grid.table.clone() },
        where_clause: String::new(),
        order_by: String::new(),
        ..grid
    };
    let statement = select_for(&by_rowid, unluminous_db::Engine::Sqlite, 10, 0);
    ...
    let statement = select_for(&by_rowid, unluminous_db::Engine::Postgres, 10, 0);
    ...
    let by_alias = Grid {
        table: unluminous_db::Table { key: vec!["_rowid_".to_owned()], ..by_rowid.table.clone() },
        ..by_rowid.clone()
    };
    assert_eq!(
        select_for(&by_alias, unluminous_db::Engine::Sqlite, 10, 0),
...
fn a_cell_shows_what_is_pending_on_it_rather_than_what_was_read() {
    ...
    assert_eq!(row, unluminous_db::Row::Keyed(vec!["1".to_owned()]));
