...
//!
//! The protocol, the tools and both transports live in `unluminous_cli::mcp`, where the client can reach
...
//!
//! The endpoint this starts drives Unluminous exactly as `unluminous-cli` does — down the control channel, with
...
pub enum State {
    ...
    Listening(u16),
    /// The setting is on, but the port is held by something else — nearly always the Unluminous in the
    /// next window, whose endpoint serves this one too.
    PortTaken(u16, String),
    /// The setting is on and there is no command channel to drive, because Unluminous was started with
    /// `--control off`.
    NoChannel,
}
...
impl State {
    ...
    pub fn message(&self) -> String {
        match self {
            State::Off => {
                "Not listening. Agents that launch Unluminous's server themselves still work.".to_owned()
            }
            State::Listening(port) => format!("Listening at {}", mcp::endpoint(*port)),
            State::PortTaken(port, problem) => format!(
                "Port {port} is already in use ({problem}). If that is another Unluminous, its server \
                 drives this window too."
            ),
            State::NoChannel => {
                "Unluminous was started with --control off, so there is no command channel for an MCP \
                 server to drive."
                    .to_owned()
            }
        }
...
pub struct Hosted {
    ...
    /// preference for which Unluminous to drive is that folder.
...
impl Hosted {
    ...
    ///
    /// `has_channel` is false when Unluminous was started with `--control off`.
    pub fn reconcile(
        &mut self,
        enabled: bool,
        port: u16,
...
mod tests {
    ...
    fn a_port_somebody_else_holds_is_reported_rather_than_swallowed() {
        ...
        assert!(hosted.state().message().contains("another Unluminous"));
    }
}
