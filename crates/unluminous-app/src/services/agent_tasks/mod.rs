...
impl UiProvider for AgentTasks {
    ...
    fn command(&mut self, command: &str, arguments: &[String]) -> Result<Answer, String> {
        ...
        match command {
            ...
            // Unluminous's rule is that everything a person can do reaches the same code from the command line.
            ...
            "epic-colour" | "epic-color" => {
                if arguments.len() < 2 {
                    return Err(
                        "say which epic and what colour: `epic-colour Unluminous #8B6BFF`".to_owned()
                    );
                }
