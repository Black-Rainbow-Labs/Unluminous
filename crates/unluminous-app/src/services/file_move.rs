...
//! move.
//!
//! It changes nothing itself. `UnluminousApp::move_path` moves the bytes and applies the plan, following
...
//!
//! ## The tier is syntactic, and it is the one Unluminous is already on
//!
...
//!
//! So the two readings are `unluminous_core::imports`' own — `specifiers_in` for the quoted family and
...
use unluminous_core::imports::{self as reading, UseLeaf};
use unluminous_core::syntax::{Grammar, ImportStyle, PathRoot};
...
/// what lets `super::a`, `crate::b::a` and `unluminous_app::b::a` all be compared with one another.
#[derive(Debug, Clone, PartialEq, Eq)]
struct Abs {
    root: PathBuf,
    segments: Vec<String>,
...
/// and those references have to follow a move as much as any other do. `unluminous-app`'s own screenshot
