...
//! `FOF_ALLOWUNDO`. That is one feature flag on the `windows-sys` dependency Unluminous already has for
...
//! new crate and cannot be compiled, let alone tested, on the machine Unluminous is built on.
//!
...
pub enum Destination {
    ...
    Bin,
    /// Gone. Nothing in Unluminous can bring it back.
    Gone,
}
...
///
/// `FOF_NOCONFIRMATION` and `FOF_SILENT` are asked for because Unluminous has already asked the question
...
fn to_the_recycle_bin(path: &Path) -> std::io::Result<()> {
    ...
    let plain = unluminous_terminal::paths::plain(&full);
...
mod tests {
    ...
    fn folder(name: &str) -> std::path::PathBuf {
        let root = std::env::temp_dir().join("unluminous-recycle").join(name);
