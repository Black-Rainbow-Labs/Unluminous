//! The channel a running Unluminous is driven down.
//!
...
//! `unluminous_cli::catalogue`, which both halves share so they cannot drift apart.
//!
...
//! language with a socket and a JSON library can drive Unluminous in three lines, so `unluminous-cli` is the
...
//! reliable way to wake a window. Two mechanisms lose it, both in code Unluminous does not own.
...
//! the event loop, where Unluminous sits. `app::cli::pump_control` already does the same thing one step
...
use unluminous_cli::instances::{self, Instance};
    ...
    fn drop(&mut self) {
        if let Some(channel) = self.reply.take() {
            let _ = channel.send(Reply::failed(
                "",
                code::FAILED,
                "Unluminous dropped the request without answering it.",
            ));
        }
    }
}
...
pub struct Server {
    ...
    /// Where the instance file was written, so the same one is removed when Unluminous stops.
    file: PathBuf,
    health: Arc<Health>,
}
...
impl Server {
    ...
    /// to start is reported and returns `None`: an Unluminous that cannot open a port is still a text
    ...
    pub fn start(folder: PathBuf, wake: Arc<dyn Fn() + Send + Sync>) -> Option<Self> {
        let listener = match bind() {
            ...
            Err(problem) => {
                eprintln!("Unluminous could not open its command channel: {problem}");
                return None;
            }
        };
        let port = match listener.local_addr() {
            ...
            Err(problem) => {
                eprintln!("Unluminous could not read its own command channel's port: {problem}");
                return None;
            }
        };
        ...
        let file = match advertise(&instance) {
            Ok(file) => file,
            Err(problem) => {
                eprintln!("Unluminous could not write its instance file: {problem}");
                return None;
            }
        };
        ...
        std::thread::Builder::new()
            .name("unluminous-control".to_owned())
            .spawn(move || accept(listener, sender, expected, wake, theirs))
            .ok()?;
        Some(Server { instance, requests, file, health })
    }

    /// Which port this Unluminous is listening on, for a test and for the status command.
...
impl Drop for Server {
    ...
    /// telling the next `unluminous-cli instances` about a window that is no longer there.
    fn drop(&mut self) {
...
/// Write the instance file, readable only by the person running Unluminous.
...
fn accept(
    ...
    for stream in listener.incoming() {
        ...
        let _ = std::thread::Builder::new()
            .name("unluminous-control-connection".to_owned())
...
/// what makes a request that reached Unluminous *after* its caller had gone answerable at all: the
...
fn read_and_queue(
    ...
    if request.token != expected {
        return Reply::failed(
            &request.command,
            code::REFUSED,
            "That is not this Unluminous's token. It is in the instance file; `unluminous-cli instances` \
             finds it.",
        );
    }
    ...
    if watched && caller_has_gone(stream) {
        ...
        return Reply::failed(
            ...
            code::TIMED_OUT,
            format!(
                "The caller had closed the connection before {command} reached Unluminous, so it was \
                 not run. {}",
...
mod tests {
    ...
    fn a_server(name: &str) -> Running {
        ...
        let folder = std::env::temp_dir().join(format!("unluminous-control-{name}"));
    ...
    fn a_caller_that_closed_the_connection_is_never_waited_for() {
        ...
        // is the one that covers a request which reached Unluminous *after* its caller gave up — where
        ...
        // been read, and that is the network's business rather than Unluminous's.
