...
//!
//! `unluminous_core::mermaid` turns a diagram's source into a `Scene`, and that is arithmetic rather than
...
use unluminous_core::mermaid::{self, Options, Problem, Scene, Theme};
use unluminous_core::metrics::{FontMetrics, LineMetrics};
use unluminous_core::CharStyle;

/// Measuring a diagram's text with the fonts `egui` will actually draw it in.
///
/// **A diagram is the one thing in the window that `unluminous-core` measures and `egui` draws.** The
/// editor measures through `TextRenderer` and paints through its own glyph atlas, so those two agree
/// by construction; a diagram's boxes are worked out in `unluminous-core` and its text is handed to
...
/// like any other, and `unluminous-core` still knows nothing about what is drawing.
...
/// Unluminous's palette, as a diagram's theme.
///
...
pub fn theme() -> Theme {
    ...
    Theme {
        ...
        node_fill: unluminous_core::mermaid::Paint::solid(of(color::title_bar())),
        node_stroke: of(color::control_border()),
        group_fill: unluminous_core::mermaid::Paint::faded(of(color::accent()), 26),
        ...
        series: [
            ...
            unluminous_core::Color::rgb(0xE0, 0x7A, 0x5F),
            unluminous_core::Color::rgb(0x9A, 0x8C, 0xE8),
            of(color::blame_old()),
        ],
    }
}
...
mod tests {
    ...
    #[test]
    fn the_theme_is_unluminouss_own_palette_and_every_series_colour_differs() {
        let theme = theme();
        assert_eq!(theme.accent, unluminous_core::Color::rgb(0x48, 0x9F, 0xF8));
    ...
    #[test]
    fn a_diagram_drawn_in_unluminouss_palette_still_keeps_every_property() {
