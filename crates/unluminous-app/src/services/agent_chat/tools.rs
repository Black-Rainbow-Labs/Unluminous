//! Unluminous's own commands, offered to the model as tools.
//!
//! **Nothing here writes a tool out.** `unluminous_cli::mcp::tools` already turns the catalogue into
...
//!
//! `instance` names which Unluminous to drive, and this chat is inside the one it would name. `timeout`
...
use unluminous_cli::mcp::tools::{self, Shape};
...
/// then read the answer back with `terminal read`. The key Unluminous takes such care never to write down
...
pub const RUNS_A_PROGRAM: &[&str] = &[
    ...
    "debug.install",
    // Starts another Unluminous on a folder of the model's choosing.
    "launch",
];

/// Whether `command` is one that hands the model a program of its own choosing.
pub fn runs_a_program(command: &unluminous_cli::catalogue::Command) -> bool {
    RUNS_A_PROGRAM.contains(&command.wire().as_str())
}

/// Every tool Unluminous offers a model, built from the catalogue.
pub fn offered() -> Vec<unluminous_chat::Tool> {
    tools::tools(SHAPE)
        .into_iter()
        .map(|tool| unluminous_chat::Tool {
...
#[derive(Debug)]
pub struct Resolved {
    pub command: &'static unluminous_cli::catalogue::Command,
    pub arguments: Map<String, Value>,
}

/// Turn a model's tool call into the request `unluminous-cli` would have sent.
///
/// The one path, so a tool call and a person typing the same command reach the same code —
/// `UnluminousApp::run_cli` — rather than two paths that agree today.
...
mod tests {
    ...
    fn every_command_held_back_is_a_command_the_catalogue_really_has() {
        ...
        for name in RUNS_A_PROGRAM {
            assert!(
                unluminous_cli::catalogue::find(name).is_some(),
