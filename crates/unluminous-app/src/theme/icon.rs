...
//!
//! - `unluminous` — the marks Unluminous shipped with: outlines at a 1.3 to 1.6 point stroke, and a solid triangle
//!   for a disclosure.
//! - `material` — heavier and rounder, filled where the Unluminous one is a stroke, and a **chevron** for a
...
///
/// Zero for the `unluminous` set, which draws no folder mark at all — the explorer has never had one, and a
...
pub fn alignment(
    ...
    for row in 0..4 {
        ...
        let x = match align {
            unluminous_core::Align::Left | unluminous_core::Align::Justify => area.left(),
            unluminous_core::Align::Center => area.left() + (full - width) / 2.0,
            unluminous_core::Align::Right => area.right() - width,
        };
        // Justified text is flush on both sides, so every line is full width except the last.
        let width = if align == unluminous_core::Align::Justify && row < 3 { full } else { width };
        let x = if align == unluminous_core::Align::Justify { area.left() } else { x };
...
/// it instead. Every icon in Unluminous is tinted where it is used — `TEXT_DIM` sitting there,
...
///
/// The one icon in Unluminous drawn in colours of its own rather than in the tint it is given. An icon
...
/// function, a bracket pair for a type — are not in the fonts Unluminous hands egui, and a missing glyph
...
/// not in the fonts Unluminous hands egui, and one that is missing renders as an empty box.
...
/// The four columns Unluminous shipped with.
