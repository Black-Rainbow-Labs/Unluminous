...
The mark is one bold serpent coiled into a simple curve shaped like the letter S — a single snake
rather than Python's own two interlocked snakes in two colours, so the mark reads as "a snake"
without reproducing the Python Software Foundation's own logo mark.
...
curl -s -X POST http://localhost:8091/image-creation/generateImageToProjectFile \
  ...
  -d '{
    "prompt": "A flat vector app icon: one bold serpent coiled into a simple thick S-curve, smooth and rounded, centred, over a single large rounded square colour swatch behind it. Two solid colours only plus the background. Bold even shapes, geometric, perfectly centred, generous margin, no outlines around the shapes. Bright cyan serpent, magenta swatch, on a completely flat solid dark navy background. Minimal, crisp, high contrast, icon design, no text, no letters, no words, no gradients, no shadows, no perspective, no white outline.",
    "negativePrompt": "text, letters, words, numbers, watermark, signature, photorealistic, 3d render, gradient, drop shadow, noise, texture, busy background, clutter, person, face, hand, realistic scales, snake eyes, two snakes, white outline, stroke, border",
    "width": 1024, "height": 1024,
    "projectId": "unluminous",
    "relativePath": "_agent_output/task-1922-language-plugins/python-icon-source.png",
...
```bash
cargo run --example plugin_icon -- _agent_output/task-1922-language-plugins/python-icon-source.png crates/unluminous-app/plugins/python
```
...
```bash
cargo run --example scale -- crates/unluminous-app/plugins/python/icon.png _agent_output/task-1922-language-plugins/python-icon-large.png 8
```
