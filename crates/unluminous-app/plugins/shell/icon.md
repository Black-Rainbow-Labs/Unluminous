...
The mark is a terminal prompt: a bold chevron pointing right beside a short thick bar, the shape
every terminal emulator draws for its own cursor, which reads as "a shell" at 32 by 32 with nothing
that could be mistaken for a letter.
...
curl -s -X POST http://localhost:8091/image-creation/generateImageToProjectFile \
  ...
  -d '{
    "prompt": "A flat vector app icon: one bold triangular chevron pointing right, beside a short thick horizontal bar like a blinking cursor, arranged together as a terminal prompt mark, centred, over a single large rounded square colour swatch behind them. Two solid colours only plus the background. Bold even shapes, geometric, perfectly centred, generous margin, no outlines around the shapes. Bright cyan chevron and bar, magenta swatch, on a completely flat solid dark navy background. Minimal, crisp, high contrast, icon design, no text, no letters, no words, no gradients, no shadows, no perspective, no white outline.",
    "negativePrompt": "text, letters, words, numbers, watermark, signature, photorealistic, 3d render, gradient, drop shadow, noise, texture, busy background, clutter, person, face, hand, keyboard, computer screen, white outline, stroke, border",
    "width": 1024, "height": 1024,
    "projectId": "unluminous",
    "relativePath": "_agent_output/task-1922-language-plugins/shell-icon-source.png",
...
```bash
cargo run --example plugin_icon -- _agent_output/task-1922-language-plugins/shell-icon-source.png crates/unluminous-app/plugins/shell
```
...
```bash
cargo run --example scale -- crates/unluminous-app/plugins/shell/icon.png _agent_output/task-1922-language-plugins/shell-icon-large.png 8
```
