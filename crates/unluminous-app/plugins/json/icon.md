...
The mark is a pair of bold curly braces, `{ }`, because that is JSON's own punctuation and the mark
every tool that reads JSON already uses for it.
...
curl -s -X POST http://localhost:8091/image-creation/generateImageToProjectFile \
  ...
  -d '{
    "prompt": "A flat vector app icon: two bold curly bracket shapes facing each other, like an opening and a closing brace, centred with generous space between them, over a single large rounded square colour swatch behind them. Two solid colours only plus the background. Bold even shapes, geometric, perfectly centred, generous margin, no outlines around the shapes. Bright cyan brackets, magenta swatch, on a completely flat solid dark navy background. Minimal, crisp, high contrast, icon design, no text, no letters, no words, no gradients, no shadows, no perspective, no white outline.",
    ...
    "relativePath": "_agent_output/task-1922-language-plugins/json-icon-source.png",
...
```bash
cargo run --example plugin_icon -- _agent_output/task-1922-language-plugins/json-icon-source.png crates/unluminous-app/plugins/json
```
...
```bash
cargo run --example scale -- crates/unluminous-app/plugins/json/icon.png _agent_output/task-1922-language-plugins/json-icon-large.png 8
```
