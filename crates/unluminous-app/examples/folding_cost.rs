...
//! applied to it.
//!
//! `cargo run --release -p unluminous-app --example folding_cost -- <file> [width]`
//!
...
use unluminous_app::services::file_kind;
use unluminous_app::services::plugins::Plugins;
use unluminous_core::folding::{self, Folds, Hidden};
use unluminous_core::{layout, relayout, FixedMetrics, ParagraphStyles, Rope, StyleSpans};
...
fn main() {
    ...
    let path = arguments
        .next()
        .map(PathBuf::from)
        .unwrap_or_else(|| PathBuf::from("crates/unluminous-app/src/app/mod.rs"));
    ...
    let spans = StyleSpans::new(rope.len_bytes(), unluminous_core::CharStyle::default());
    ...
    let read_after = match reading {
        unluminous_core::folding::Reading::Code(grammar) => folding::tokens(&edited, grammar),
        _ => unluminous_core::folding::Tokens::default(),
    };
