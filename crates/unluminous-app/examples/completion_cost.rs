...
//!
//! `cargo run --release -p unluminous-app --example completion_cost -- [folder] [stem]`
//!
//! It drives a **real window** — the same `UnluminousApp` the application is, with the project's index
...
use unluminous_app::app::UnluminousApp;
...
fn main() {
    ...
    let bare = unluminous_core::Grammar { export_keyword: None, ..grammar.clone() };
    ...
    let samples: &[&str] = match grammar.imports {
        Some(unluminous_core::syntax::ImportStyle::Path) => &[
            "\nuse ",
            "\nuse unluminous_core::",
            "\nuse unluminous_core::completion::",
            "\nuse unluminous_core::completion::Can",
        ],
        Some(unluminous_core::syntax::ImportStyle::Quoted) => {
            &["\nimport { A } from './", "\nimport { A } from './la", "\nimport { La } from './"]
        }
        None => &[],
    };
