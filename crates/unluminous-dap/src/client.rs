...
//! process Unluminous started, the same as any run.
...
///
/// `UNLUMINOUS_DAP_TRACE=1` switches it on, read once. It exists because a protocol client with no way to
...
fn tracing() -> bool {
    ...
    *TRACING.get_or_init(|| {
        std::env::var("UNLUMINOUS_DAP_TRACE").is_ok_and(|value| !matches!(value.as_str(), "" | "0"))
    })
}
...
pub enum Reply {
    /// One message the adapter sent. Boxed for the reason `unluminous_git::Reply::Snapshot` is: a
...
impl Client {
    ...
    pub fn start(command: &AdapterCommand, waker: Waker) -> Result<Self, String> {
        ...
        // explains itself, and swallowing that is the one thing `unluminous-git`'s rule about git's
        // stderr says never to do.
        if let Some(errors) = connection.errors {
            let said = sender.clone();
            let woken = waker.clone();
            std::thread::Builder::new()
                .name("unluminous-dap-errors".to_owned())
    ...
    pub fn adopt_child(&mut self, command: &AdapterCommand, waker: Waker) -> Result<(), String> {
        ...
        std::thread::Builder::new()
            .name("unluminous-dap-child".to_owned())
            .spawn(move || read_child_frames(reader, sender, waker))
            .map_err(|problem| {
                format!("Unluminous could not start a thread to read the child session: {problem}")
            })?;
    ...
    ///
    /// What the screenshot tests use, exactly as `unluminous_terminal::Session::detached` is what the
    ...
    pub fn is_running(&mut self) -> bool {
        match self.child.as_mut() {
            ...
            // An adapter Unluminous did not start is somebody else's process, so its being there is not
...
mod tests {
    ...
    #[test]
    fn an_adapter_unluminous_did_not_start_is_never_reported_as_dead() {
        let (sender, receiver) = std::sync::mpsc::channel();
        let mut client = Client {
            replies: receiver,
            writer: Box::new(Vec::new()),
