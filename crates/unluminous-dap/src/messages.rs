//! The messages Unluminous sends and the ones it reads, as Rust types with the JSON written by hand.
//!
//! **Only what Unluminous uses is modelled.** The protocol is large and deliberately additive — an
...
//! speak, so what crosses is a value. `unluminous-cli/src/protocol.rs` made the same choice about Unluminous's
//! own control channel.
//!
    ...
    pub fn arguments(&self) -> Option<Value> {
        Some(match self {
            Request::Initialize { client_id, lines_start_at_one } => json!({
                "clientID": client_id,
                "clientName": "Unluminous",
                "adapterID": "unluminous",
                "locale": "en",
...
/// so Unluminous's whole cost for two features the reference editor has is two `Option<String>`s and the modal that
/// edits them.
...
///
/// **Unluminous draws this rather than its own hope.** An adapter that moved a breakpoint to the next
...
///
/// **Every optional feature asks this first**, so Unluminous never sends what an adapter did not offer
/// and a control whose capability is absent is absent — the rule the `F` button already follows.
/// A capability this file does not name is one nothing in Unluminous gates on.
#[derive(Debug, Clone, Default, PartialEq, Eq)]
pub struct Capabilities {
    ...
    /// The named exception filters this adapter offers, as `(filter, label, default)`. Unluminous holds
...
pub struct Stopped {
    ...
    /// The sentence an adapter adds for an exception, which is better than anything Unluminous could say.
...
/// Everything an adapter can say that Unluminous reads.
///
...
pub enum Message {
    ...
    Response {
        ...
        command: String,
        success: bool,
        /// The adapter's own message when it refused. **Never replaced with one of Unluminous's**: a
...
fn read_reverse_request(value: &Value) -> Message {
    ...
    Message::RunInTerminal {
        seq: value.get("seq").and_then(Value::as_i64).unwrap_or(0),
        // `integrated` or `external`. Unluminous has one terminal and runs both in it, which is what
...
/// one often cannot say which process it became. Unluminous is one of those — `unluminous_terminal::Session`
...
pub fn run_in_terminal_response(
    ...
    match (started, process) {
        ...
        (false, _) => {
            message.insert(
                "message".to_owned(),
                json!("Unluminous could not start the program in its run tile."),
            );
        }
    }
    Value::Object(message)
...
mod tests {
    ...
    /// logs, so Unluminous's whole cost for two of the reference editor's features is two optional strings.
    #[test]
    ...
    /// The adapter's own refusal is carried whole, because nothing in Unluminous could say it better.
    #[test]
    ...
    #[test]
    fn an_event_unluminous_does_not_read_is_carried_rather_than_dropped() {
